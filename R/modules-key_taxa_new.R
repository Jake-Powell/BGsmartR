...
#' @export
#'
create_key_taxa_report <- function(enriched_report,
                                   collection = NULL,
                                   wgsrpd3 = NULL,
                                   wcvp = NULL,
  ...
  if(report_kind == 'interactive'){
    ...
    if(is.null(output_file) & is.null(collection)){
      output_file = 'key_taxa_static.docx'
    }
    else if(is.null(output_file) & !is.null(collection)){
      output_file = paste0(collection, '_key_taxa_static.docx')
    }
  }else{
  ...
  if(report_kind == 'static'){
    if(is.null(reference_docx)){
      rmarkdown::render(paste0(system.file(package = "BGSmartR"), "/markdown_reports/Key_taxa_report.Rmd"),
