...
#' create_basic_stats_interactive()
#'
#' @param enriched_report enriched report
#' @param min_year The first year to consider for retrospective review.
#' @param collection The name of the collection
...
#' @export
#'
create_basic_stats_interactive <- function(enriched_report, min_year=NULL, collection = NULL, output_file = NULL){
  # 1) Setup.
  # A) Check enriched report.
  ...
  # 2) Render basic stats_static document.
  rmarkdown::render(paste0(system.file(package = "BGSmartR"), "/markdown_reports/Basic_Stats_interactive.Rmd"),
                    params = list(enriched_report = enriched_report,
                                  min_year = min_year,
                                  collection = collection),
                    output_file = output_file,
