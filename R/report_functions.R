...
single_trend_plot <- function(years, values, name, report_kind, separate_figure_folder){
  ...
  if(report_kind == 'interactive'){
    ...
    if(separate_figure_folder){
      htmlwidgets::saveWidget(fig, file = paste0(separate_figure_folder,'/',name,'_trend.html'),selfcontained = T)
    }
    return(fig)
  }
  if(report_kind == 'static'){
    p =  ggplot2::ggplot(data = df, ggplot2::aes(x = years, y = values)) +
      ggplot2::geom_line(width = 1.1) +
      ggplot2::labs(title="",
           x ="Year",
    ...
    if(separate_figure_folder){
      ggplot2::ggsave(plot = p, filename = paste0(separate_figure_folder, '/', name, '_trend.pdf'),device = 'pdf', scale = 1)
    }
    return(p)
  }
...
group_trend_plots <- function(years, data, name, report_kind, separate_figure_folder, data_type, split = FALSE, group_order = NULL){
  ...
  dd = stats::reshape(tt, idvar = "year", varying = list(2:ncol(tt)),
  ...
  dd_prop = stats::reshape(tt_prop, idvar = "year", varying = list(2:ncol(tt)),
                    v.names = "count", timevar = "group", times = names(tt)[-1], direction = "long")
  dd_prop$group = factor(dd_prop$group, levels = unique(dd_prop$group))

  if(report_kind == 'static'){
    rlang::is_installed("ggpubr")
    p1 = ggplot2::ggplot(dd, ggplot2::aes(x=year, y = count, group = group, color = group)) +
      ggplot2::geom_line(linewidth  =1.1 ) +
      ggplot2::guides(color=ggplot2::guide_legend(title=paste0(name))) +
      ggplot2::labs(title="",
           x ="Year",
    ...
    p2 = ggplot2::ggplot(dd_prop, ggplot2::aes(fill=group, y=count, x=year)) +
      ...
      ggplot2::labs(title="",
           x ="Year",
           y = "Percentage (%)") +
      ggplot2::guides(fill=ggplot2::guide_legend(title=paste0('Provenance')))
  ...
  if(report_kind == 'interactive'){
    ...
    fig <- plotly::plot_ly(dd, x = ~year, y = ~count, color = ~group, type = 'scatter', mode = 'lines', colors = interactive_colour(length(unique(dd$group))))

    fig <- fig |> plotly::layout(yaxis = list(title = paste0("Number of ",data_type |> tolower(),"")),
                         xaxis = list(title = "Year"))
    fig <- fig |> plotly::layout(hovermode = 'x unified')
    ...
    fig2 <- plotly::plot_ly(dd_prop, x = ~year, y = ~count, color = ~group, type = 'bar',
    ...
    fig2 <- fig2 |> plotly::layout(yaxis = list(title = paste0("Percentage")),
                           xaxis = list(title = "Year"))
    fig2 <- fig2 |> plotly::layout(hovermode = 'x unified',
                           barmode = 'stack',
                           bargap =0,
    ...
    if(separate_figure_folder){
      htmlwidgets::saveWidget(fig2, file = paste0(separate_figure_folder,'/',name,'_proportional_trend.html'),selfcontained = T)
    }
  }
...
group_trend_plots_pair <- function(years, data, name, report_kind, separate_figure_folder, data_type, split = FALSE, group_order = NULL, color_binary = interactive_colour(4, type = 'trend')){
  ...
  if(report_kind == 'interactive'){
    ...
    fig <- plotly::plot_ly(tt,
      ...
      plotly::add_trace(y = tt[,3],
                name = names(tt)[3],
                line = list(color = color_binary[2]),
                visible='legendonly'
      )|>
      plotly::layout(yaxis = list(title = paste0("Number of ",data_type |> tolower(),"")),
             xaxis = list(title = "Year"),
             hovermode = 'x unified')
    fig <- fig |> plotly::layout(hovermode = 'x unified')
