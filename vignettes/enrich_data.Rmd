...
# plant search url with germplasm_type = plant and excluding cultivars.
# We split this into two sections so we can add the family search in the middle.
web_address = c('https://plantsearch.bgci.org/search?','filter[germplasm_type]=plant&filter[cultivar_status]=exclude&sort=name')

# Get the urls of searching each family.
family_search_urls = paste0(web_address[1], 'filter[family]=',families,'&', web_address[2])
...
for(i in 1:length(families)){
  ...
  # navigate to the current family search.
  rmDr$navigate(family_search_urls[i])
  
  # Go to sleep to allow the table to load.
  Sys.sleep(5)
  
  # Retrieve how many matches we found. Be getting the text at the top of the table "XX Taxa Found". And stripping out the number XX. (use google chrome inspect to get the xpath of this element)
  ...
  # Depending on the number found we have to do different things:
  
  # if there are no records we don't need to add anything to the output
  if(number_found == 0){
    next
   }
  # if there are greater then 500 records we can't extract them all. So we won't extract any.
  if(number_found > 500){
     warning(paste0('More than 500 found for family = ', families[i]))
    next
   }
  # If we have between 101 and 500 records we need to load all the records before downloading the table
  # (only 100 are originally loaded)
  ...
  # If we have between 1 and 100 records we can just carry on...
...
# We have finished and can close the Selenium server.
remote_driver$server$stop()

# format the output.
output = data.frame(output)
names(output) = c('Taxon name', 'Family', 'Name Accepted', '# of Ex Situ Sites')
