...
In BGSmartR we use an opinionated method to match taxonomic records to Plants of the World Online (Link),  IUCN Red List (Link) and BGCI. Our method is automatic and does not require any manual alterations. Moreover, rather than rely on "fuzzy" matching in difficult cases we have a criteria which is used to find the best possible match. 
...
To match your collections data to POWO (and IUCN + BGCI) the algorithm and been wrapped into the function `enrich_report()`, which can be used to match and enrich your collection without concerning yourself with the method applied. See vignette XX to see the function in action. 
...
After each step where matching occurs (2,3,5,6,7,8,9,10) the list of names to match is reduced by the names that we found. In other words, once a match is found for a single taxon name no more steps are taken. For example, if we found adding hybridisation leads to a match to POWO we will not search for typos later on with the inputted taxon name. 
...
This is performed using the function `known_not_in_wcvp()`, which returns the index of any taxon name which contains any of the above patterns. For these taxon names we set the output message to `(Not in POWO <known not to be in POWO>)`. 

Below we provide an example of `known_not_in_wcvp()`.

```{r known_not_in_wcvp}
taxon_names = c('Acalypha fruticulosa', 'Asclepias sp.', "Abies umbilicata 'Some Cultivar'", 
                'Ammi copticum cv.')
known_not_in_wcvp(taxon_names)
```

We see that the function returns the indices (starting from 1) of the taxon names which contains the patterns outlined above. 
***
...
This step is performed using the function `match_single_wcvp()`. 
...
# Perform the match
match = match_single_wcvp(taxon_names = taxon_names, wcvp = wcvp, wcvp_search_index = wcvp_search_index_single)
match
```
...
If we find a match we also provide the powo_id (e.g. `108437-3`) which can be used to search for the plant directly on POWO's website.
...
We that that each taxon name an author, a taxon status and an accepted plant name. These quantities can be used to decide which record we should match to. 

##### Matching Criteria.

This can be split into two parts author matching and choosing records. We will begin with choosing records, which is performed by the function `match_taxon_status()`. The algorithm is as follows:

- If we have a single record, return this record.
- If we have more than one record:
  - If all plants have the same accepted plant, return the record that is accepted, otherwise return the first record. 
  - Else,  Consider the taxon status.
    - If exactly one record has Accepted status return this. 
    - If none has accepted status and exactly one has Synonym status, return this.
    - If there are multiple accepted or multiple synonym status return `NA` with the message `multiple best taxon status, do not match`.
    - If none are accepted or synonyms return `NA` with the message `no accepted or synonym`.
    
Author matching is attempted in 4 ways, illustrated below where the first author is POWO and second the current plants. 

1) EXACT author matching. POWO = `Sobol. ex Rupr.`, current = `Sobol. ex Rupr.`.
2) POWO's author contained in current plant's author. POWO = `Sobol.`, current = `Sobol. ex Rupr.`.
3) current plant's author contained in POWO's author. POWO = `Sobol. ex Rupr.`, current = `Sobol.`.
4) Split POWO's author into words explore is each word is contained in current plant's author. POWO = `J.Lee ex Steud.` current = `James Lee`.

After each step if a match is found we stop and do not try any further author matching.

Note that author matching (especially partial author matching) may lead to multiple potential authors. Therefore, after trying each step of author matching we run `match_taxon_status()`.

The combination of author matching and taxon matching is performed by the function `get_match_from_multiple()` for a single taxon name and author combination. To author match for multiple records we use the function `match_mult_wcvp()` which depends on `get_match_from_multiple()`.

An example of author matching is provided below

```{r example mult author matching}
taxon_names = c('Abies taxifolia', 'Abies taxifolia', 'Abies taxifolia', 'Abies taxifolia', 'fake name')
taxon_authors = c('Poir.', '', 'Desf. and Raf.', 'Desf. and Poir.',  'Dave')

match_mult_wcvp(taxon_names, taxon_authors, wcvp, wcvp$wcvp_names$single_entry == F, show_progress = F)
```
...
# Perform the match
match = match_rm_autonym(taxon_names = taxon_names, taxon_authors = taxon_names, wcvp = wcvp)
match
```
...
# Perform the match
match = match_hybrid_issue(taxon_names = taxon_names, taxon_authors = taxon_names, wcvp = wcvp)
match
```
...
# Perform the match
match = add_splitter(taxon_names = taxon_names, taxon_authors = taxon_names, wcvp = wcvp)
match
```
...
# Perform the match
match = match_typos(taxon_names = taxon_names, taxon_authors = taxon_names, wcvp = wcvp, typo_method = 'full')
match
```
...
taxon_names = c('Adina nobilis', 'Acalypha gracilens var. fraseri', 
                'Andira zehntneri')
match = match_single_wcvp(taxon_names,wcvp,wcvp_search_index_single)
match
