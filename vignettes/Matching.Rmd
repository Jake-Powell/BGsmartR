...
This is performed using the function `no_match_cultivar_indet()`, which returns the index of any taxon name which contains any of the above patterns. For these taxon names we set the output message to `(Not in POWO <known not to be in POWO>)`. 
...
# 
# match_multiple(taxon_names, taxon_authors, wcvp$wcvp_names, wcvp$wcvp_names$single_entry == F, show_progress = F)
```
