...
We find that `r enrich_collection$Enrich_is_tree |> sum(na.rm = T)` records are trees.
...
Within BGsmartR we can create reports for:

- Trends 
- Turnover
- etc

To run these reports we require the collection's information to be enriched with data from POWO, IUCN RedList, BGCI GlobalTreeSearch and BGCI's PlantSearch.
...
- "wcvp" World checklist of vascular plants database formatted using BGsmartR.
- "detailed_IUCN_redlist" IUCN redlist information with history. 
