...
In the living collection there are `r format(no_threatened_items_existing, big.mark=',')` items corresponding to  `r format(no_threatened_accessions_existing, big.mark=',')` accessions of existing threatened plants. The number of threatened species existing in the living collection is summarised below. Note that here species is defined as the genus species pair and taxa is the taxonomic name and author (thus including subspecies, variety, forma).
...
```{r, How many threatened cateogories in the living collection - Table}
...
if(report_kind == 'interactive'){
  cat_table |>  DT::datatable(extensions = 'Buttons',
                  rownames = FALSE,
                  options = list(scrollX = TRUE,
...
In this section we explore how the number of threatened species in the living collection has varied over time. For each year we use the accession year and item status date to determine which items in the collection are existing on the 31st of December each year.
...
Recall that we use WCVP to determine the geographic distribution of taxa and we use a combination of WCVP and IUCN red list to determine the threatened status. Therefore, it is possible that we find a threatened taxa (using red list) that is not found in WCVP, and thus we do not have a geographic distribution for these taxa. In the living collection there are `r length(unique(threatened_existing$best_name[which(is.na(threatened_existing$geography_codes))]))` taxa that are classed as threatened without geographic information provided by WCVP, these can be viewed below.
...
`r if(report_kind == 'interactive'){"##### Maps of threatened species held in the living collection {.tabset}"}`
...
`r if(do_survival){"Next we give the median surivival times for threatened and non-threatened plants in the collection."}`
...
### Breakdown by threatened category
...
```
```{r  duel pie charts, eval = report_kind == 'interactive' & do_over_time}
...
In this section we are exploring the red list category of threatened species in the living collection currently and historically. 
...
`r if(report_kind == 'interactive'){flex_data_table}`

`r if(report_kind == 'interactive'){"##### {-}"}`


`r if(have_redlist & do_over_time){"We will now look at how the threatened categories of native plants in the living collection has changed over time."}`

`r if(!do_over_time){"We do not have the information to explore how hreatened categories of native plants in the living collection has changed over time."}`
