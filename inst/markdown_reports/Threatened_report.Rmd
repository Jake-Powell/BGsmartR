...
palette = 'Oranges'
detailed_IUCN = detailed_IUCN_redlist
rm(detailed_IUCN_redlist)
AAA <<- detailed_IUCN
...
# earliest_allowable_record = 1846
```
...
```{r, functions for turnover, eval = T}
...
plots_turnover <- function(turnover, trend, report_kind, text = '', separate_figure_folder, data_type = 'items', colors = c('#f46d43', '#848484')){
  years = turnover$Year
...
enriched_report$ProvenanceCode[is.na(enriched_report$ProvenanceCode)] = 'Unknown'
...
```{r check for threatened, warning=NA}
threatened = enriched_report[which(enriched_report$threatened == 'Threatened'),]

no_threatened = nrow(threatened) == 0
if(no_threatened){
  warning('There are no threatened plants!')
}

# Create cleaned Genus Species (Note here that we must match to either POWO or redlist to be classed as threatened therefore we don't need to use the name from the report)
GenusSpecies = rep(NA,nrow(threatened))
GenusSpecies[!is.na(threatened$POWO_genus)] = paste0(threatened$POWO_genus[!is.na(threatened$POWO_genus)], ' ', threatened$POWO_species[!is.na(threatened$POWO_genus)] )
GenusSpecies[is.na(GenusSpecies)] = threatened$redList_match_taxon_name[is.na(GenusSpecies)] |> stringr::str_split(' ') |> lapply(function(x){paste0(x[1], ' ', x[2])}) |> unlist()
threatened$BestGenusSpecies = GenusSpecies

# Create Full name for the taxa of threatened species.
FullName = rep(NA,nrow(threatened))
FullName[!is.na(threatened$POWO_genus)] = paste0(threatened$POWO_taxon_name[!is.na(threatened$POWO_genus)], ' ', threatened$POWO_taxon_authors[!is.na(threatened$POWO_genus)])
FullName[is.na(FullName)] = paste0(threatened$redList_match_taxon_name[is.na(FullName)], ' ', threatened$redList_match_authors[is.na(FullName)])
threatened$FullName = FullName
```

`r if(no_threatened){"There are no threatened plants."}`
`r if(no_threatened){knitr::knit_exit()}`

```{r,  get threatened only information}
max_year = format(Sys.Date(), '%Y') |> as.numeric()

enriched_report2 <<- enriched_report
threatened2 <<- threatened
# threatened including non-POWO matches.
...
no_threatened_species_existing = threatened_existing$BestGenusSpecies |> unique() |> length()
no_threatened_taxa_existing =  threatened_existing$FullName |> unique() |> length() 
...
over_time_info = lapply(threatened_existing_each_year, function(x){
  ...
  no_taxa = garden_current$FullName |> unique() |> length()
  no_species = garden_current$BestGenusSpecies |> unique() |> length()
...
over_time_info = lapply(plant_existing, function(x){
  ...
  no_items = nrow(garden_current)
  no_accessions = length(unique(garden_current$AccNoFull))
  no_taxa = length(unique(paste0(garden_current$POWO_taxon_name, ' ', garden_current$POWO_taxon_authors)))
  no_species = length(unique(paste0(garden_current$POWO_genus, garden_current$POWO_species)[!is.na(garden_current$POWO_plant_name_id)]))
...
# threatened_accessions = threatened_accessions[,match(c('AccNoFull', 'TaxonNameFull', 'powo_name', 'threatened', 'threatened_category', 'endemic', 'geography_codes', 'ProvenanceCode', 'no_gardens', 'PLantSearchToPOWO_no_gardens'), names(threatened_accessions))]
# names(threatened_accessions) = c('Accession Number', 'Original Taxonomic Name and Author', 'POWO Taxonomic Name and Author', 'Threatened', 'Red list category', 'endemic', 'Geographic codes..', 'Provenance', 'Number of Global Collections Held in (By original name)', 'Number of Global Collections Held in (by POWO name)')
# dir.create(output_dir,showWarnings = F,recursive = T)
# writexl::write_xlsx(threatened_accessions, path =paste0(output_dir,'/',collection,'_threatened_accessions_list.xlsx'))
...
threatened_existing_species = threatened_existing[match(unique(threatened_existing$BestGenusSpecies),
                                                               threatened_existing$BestGenusSpecies),]
...
threatened_existing_taxa = threatened_existing[match(unique(threatened_existing$FullName),
                                                            threatened_existing$FullName),]
cat_taxa = table(threatened_existing_taxa$threatened_category)
...
threat_match_IUCN = sum(!is.na(detailed_IUCN$POWO_plant_id))
prop_threat_match_IUCN = round(threat_match_IUCN/nrow(detailed_IUCN)*100,digits = 2)
# number of threatened per geographic area. 
threatened_per_region = unlist(lapply(location_data$code, function(code){
  sum(grepl(code, detailed_IUCN$geography))
}))
print("you're a cunt")
...
if(!is.null(wcvp)){
  ...
  missing_regions = missing_regions[order(as.numeric(missing_regions$`Number of Threatened Species`), decreasing = T),]
   ...
   info = info[info$id %in% detailed_IUCN$POWO_plant_id,]
   ids = info$id
  ...
  IUCN_indo = detailed_IUCN[match(wcvp_wantedo$plant_name_id, detailed_IUCN$POWO_plant_id),]
...
if(nrow(duplication_items) != 5){
  no_gap = data.frame(Duplication = c(1,2,3,4,'>4'), Items = rep(0,5))
no_gap$Items[match(duplication_items$Duplication|> as.vector(), no_gap$Duplication)] = duplication_items$Items
duplication_items = no_gap
}
...
if(nrow(duplication_accessions) != 5){
  no_gap = data.frame(Duplication = c(1,2,3,4,'>4'), Accessions = rep(0,5))
no_gap$Accessions[match(duplication_accessions$Duplication|> as.vector(), no_gap$Duplication)] = duplication_accessions$Accessions
duplication_accessions = no_gap
}
