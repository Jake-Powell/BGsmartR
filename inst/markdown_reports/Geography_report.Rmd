...
To get geographic information the living collection has been enriched with information from [Plants of the World Online (POWO)](https://powo.science.kew.org). Only records that are matched to POWO have geographic information. This is true for `r percent_matched`% (`r format(no_powo_matched ,big.mark = ',')`)  of the items in the living collection.
...
#Get the number of trees for each location.
tree_info = wanted_info[which(wanted_info$Enrich_is_tree |> as.logical()),]
no_tree = unlist(lapply(All_locations, function(x){
  sum(grepl(x, tree_info$geography_codes))
}))
...
if(export_data){
  ...
  names(l) = c('BRU level 3 code',
               ...
               'Represented by Endemic Taxa',
               'Number of Threatened Taxa',
               'Represented by Threatened Taxa',
               'Number of Trees (Taxa)',
...
if(is.null('tree_species_per_region')){
  ...
  # if(!exists('tree_species_per_region') || is.null(tree_species_per_region)){
  no_tree_accepted_species_wcvp = unlist(lapply(All_locations, function(x){
    sum(grepl(x, wcvp_geography_tree$codes))
  }))
  tree_species_per_region = data.frame(code = All_locations, no_tree_accepted_species_wcvp = no_tree_accepted_species_wcvp)
  # }
}
...
location_data$tree_species_per_region = tree_species_per_region[,2]
...
```{r table - interative}
# l_data <<- location_data
# show_table = location_data[,-c(1,6,8,10,12,13,14,15)]
show_table = location_data |> dplyr::select( "name", "level_2", "level_1", "no_species",
                                             "no_endemic", "no_threatened", "no_tree")
# show_table = show_table[,-c(ncol(show_table)-1, ncol(show_table))] # Do I ever need this?
...
## Taxa in the living collection

<!-- ### BRU level 3 areas represented in the living collection -->

```{r, BRU level 3 areas represented in the living collection - regions not represented.}
...
if(!is.null(wcvp)){
  ...
  names(missing_regions) = c('Region', 'Level 2 region','Level 1 region', 'Number of Taxa')
  missing_regions <- missing_regions[order(as.numeric(missing_regions$`Number of Taxa`), decreasing = T),]
...
```{r, BRU level 3 areas represented in the living collection - static figure, echo = FALSE, fig.fullwidth=FALSE, fig.dim = c(10, 6), eval = FALSE}
...
<!-- ### Number of species in the living collection from each BRU level 3 area. -->

```{r, Number of species in the living collection from each BRU level 3 area, fig.fullwidth=FALSE, fig.dim = c(10, 6)}
if(report_kind == 'static'){
...
```{r, Number of species in the living collection from each BRU level 3 area - regions most species, results='asis'}
if(report_kind == 'static'){
...
Of 369 regions described by TDWG geographical codes, 368 of them have native taxa in POWO. The exception is Bouvet Island which has none. 

`r if(report_kind == 'interactive'){"##### Maps of accepted taxa held in the living collection {.tabset}"}`
...
plot_int_info$percent  = round(plot_int_info$no_species/plot_int_info$WCVP_total*100,digits = 2)
 text = paste0('Region: ', plot_int_info$name, '<br>', 
                ...
                'Taxa in the LC:', plot_int_info$no_species, '/', plot_int_info$WCVP_total, '<br>',
                'Taxa from Region in LC:', round(plot_int_info$no_species/plot_int_info$WCVP_total*100,digits = 2), '%')
  plot_int_info$hover = text
  plot_int_info <<- plot_int_info
  ...
  if(separate_figure_folder){
    htmlwidgets::saveWidget(fig, file = paste0(figures_dir, '/','Geography-Representation_of_Taxa.html'))
  }
  suppressWarnings(fig)

```

`r if(report_kind == 'interactive'){"###### By Number of Accepted Taxa Held"}`
...
#   plot_int_info$hover = text
 ...
 colscale_0_1 = as.numeric(unlist(lapply(col_scale, function(x){x[[1]]})))
  ...
  if(separate_figure_folder){
    htmlwidgets::saveWidget(fig, file = paste0(figures_dir, '/','Geography-Number_of_Taxa.html'))
  }
  suppressWarnings(fig)

```

`r if(report_kind == 'interactive'){"###### By Proportion of Accepted Taxa held"}`
...
In the living collection `r percent_rep`% of the BRU level 3 regions are represented, with `r length(no_rep)` regions un-represented.

`r if(length(no_rep) > 0){"Regions with no taxa in the living collection are outlined below, together with the total number of taxa native to each region."}`
...
if(report_kind == 'interactive'){
 ...
 missing_regions |>  DT::datatable(extensions = 'Buttons',
                  ...
                  options = list(scrollX = TRUE,
                                 ...
                                 dom = 'Btp',
                                 buttons = c('copy', 'csv', 'excel', 'pdf'),
                                 paging= ifelse(nrow(missing_regions) > 10, T,F),
                                 ordering=T),
                  filter = list(position = "top"))
}
...
`r if(length(no_rep) > 0){"Further information about the plants from the missing regions in the living collection can be downloaded using the button below."}`

```{r link to download plants from missing regions, eval = report_kind == 'interactive' & length(no_rep) > 0 & do_download}
...
Note that not every region has endemic taxa In particular, within WCVP there are `r sum(location_data$no_endemic_accepted_wcvp_species == 0)` regions that have no endemic taxa For example: `r paste0(location_data$name[location_data$no_endemic_accepted_wcvp_species == 0][1:5], collapse = ', ')`. Therefore, those regions are removed prior to creating the maps.
...
`r if(report_kind == 'interactive'){"##### Maps of Endemic taxa held in the living collection {.tabset}"}`
...
text = paste0('Region: ', plot_int_info$name, '<br />', 
              ...
              '# Taxa in LC: ', plot_int_info$no_endemic_species, '/',plot_int_info$no_endemic_accepted_wcvp_species,'<br />',
...
if(separate_figure_folder){
  htmlwidgets::saveWidget(fig, file = paste0(figures_dir, '/','Geography-Representation_of_Endemic_Taxa.html'))
}
suppressWarnings(fig)

```

`r if(report_kind == 'interactive'){"###### By Number of Endemic Taxa Held"}`
...
`r if(report_kind == 'interactive'){"###### By Proportion of Accepted Endemic Taxa held"}`
...
In the living collection `r percent_rep`% of the BRU level 3 regions are represented by endemic taxa, with `r length(no_rep)` regions un-represented. 

`r if(length(no_rep) > 0){"Regions with no endemic taxa in the living collection are outlined below, together with the total number of endemic plants in each region."}`
...
if(report_kind == 'interactive'){
  ...
  missing_regions |>  DT::datatable(extensions = 'Buttons',
                  rownames = FALSE,
                  options = list(scrollX = TRUE,
                                 pageLength = min(10,nrow(missing_regions)),
                                 dom = 'Btp',
                                 buttons = c('copy', 'csv', 'excel', 'pdf'),
                                 paging=T,
                                 ordering=T),
...
```{r link to download plants from missing endemic regions, eval = report_kind == 'interactive' & length(no_rep) > 0 & do_download}
  wcvp_wanted |>
  download_this(
    output_name = "WCVP endemic plants from non-represented regions",
    output_extension = ".xlsx",
    button_label = "Download endemic taxa from non-represented regions with endemic taxa (xlsx)",
    button_type = "default",
    has_icon = TRUE,
    icon = "fa fa-save",
    class = "hvr-sweep-to-left",
...
threat_match_IUCN = sum(!is.na(detailed_IUCN_redlist$POWO_plant_id))
prop_threat_match_IUCN = round(threat_match_IUCN/nrow(detailed_IUCN_redlist)*100,digits = 2)
# number of threatened per geographic area. 
threatened_per_region = unlist(lapply(location_data$code, function(code){
  sum(grepl(code, detailed_IUCN_redlist$geography))
}))
...
if(!is.null(wcvp)){
  ...
  missing_regions = missing_regions[order(as.numeric(missing_regions$`Number of Threatened Species`), decreasing = T),]
   info <- wcvp_geography[grep(paste0(missing_codes, collapse = '|'),wcvp_geography$codes),]
   info = info[info$id %in% detailed_IUCN_redlist$POWO_plant_id,]
   ids = info$id
  region_wanted = rep('',nrow(info))
  ...
  wcvp_wantedo$web_link = paste0("https://powo.science.kew.org/taxon/urn:lsid:ipni.org:names:",wcvp_wantedo$powo_id)
  wcvp_wantedo$plant_native_to =region_wanted
  IUCN_indo = detailed_IUCN_redlist[match(wcvp_wantedo$plant_name_id, detailed_IUCN_redlist$POWO_plant_id),]
...
if(report_kind == 'interactive'){
  ...
  threatened_count_table |>  DT::datatable(extensions = 'Buttons',
                  rownames = FALSE,
                  options = list(scrollX = TRUE,
                                 pageLength = min(10,nrow(threatened_count_table)),
                                 dom = 'Bt',
                                 buttons = c('copy', 'csv', 'excel', 'pdf'),
                                 paging=T,
                                 ordering=T))
...
`r if(report_kind == 'interactive'){"##### Maps of threatened taxa held in the living collection {.tabset}"}`

`r if(report_kind == 'interactive'){"###### Has a threatened taxa"}`
...
`r if(report_kind == 'interactive'){"###### By number of threatened taxa held"}`
...
`r if(report_kind == 'interactive'){"###### By proportion of threatened taxa held"}`
...
In the living collection `r round(with_threatened/sum(threatened_per_region > 0)*100,digits = 2)`% of regions with native threatened taxa are represented. The regions without representation are outlined below together with the number of threatened taxa in each region. 
...
```{r link to download plants from missing threatened regions, eval = report_kind == 'interactive' & length(no_rep) > 0 & do_download}
  wcvp_wantedo |>
  download_this(
...
```{r Tree Species map -  data}
tree = enriched_report[which(enriched_report$Enrich_is_tree |> as.logical()),]
# Get all BRU level 3 location codes.
All_locations = wgsrpd3$LEVEL3_COD
...
if(!is.null(wcvp)){
  ...
  names(missing_regions) = c('Region', 'Level 2 region','Level 1 region', 'Number of Tree Species')
  missing_regions = missing_regions[order(missing_regions$`Number of Tree Species`, decreasing = T),]
  ids = wcvp_geography$id[wcvp_geography$codes %in% location_data[which(location_data$no_tree_items == 0 & location_data$no_tree_accepted_wcvp_species !=0), match(c('code'), names(location_data))]]
...
Next we produce maps showing where in the world the trees in the living collection are from. 


```{r, Number tree Species in the LC - figure new, fig.fullwidth=FALSE, fig.dim = c(10, 6), eval = report_kind == 'static'}
top10_name = plot_info_tree$LEVEL3_NAM[order(plot_info_tree$no_tree_species, decreasing = TRUE)][1:10]
top10_value = plot_info_tree$no_tree_species[order(plot_info_tree$no_tree_species, decreasing = TRUE)][1:10]
top10 = data.frame(region = top10_name, no_tree = top10_value)

# plot_info_tree = plot_info_tree[!plot_info_tree$no_tree_accepted_wcvp_species ==0,]
###################################################################
# Plot of the representation of tree species in each geographic region.
# Regions that have no tree plants are set to NA.
plot_info_tree$rep_tree[plot_info_tree$rep_tree == TRUE] = 'Yes'
plot_info_tree$rep_tree[plot_info_tree$rep_tree == FALSE] = 'No'

if('no_tree_accepted_wcvp_species' %in% names(plot_info_tree)){
  plot_info_tree$rep_tree[plot_info_tree$no_tree_accepted_wcvp_species == 0] = 'No tree'
}
plot_info_tree$Represented = plot_info_tree$rep_tree

p = ggplot(plot_info_tree)+
  geom_sf(aes(fill=Represented),  col="transparent")+
  stat_sf_coordinates(aes(col=Represented), show.legend = FALSE)+
  coord_sf(expand=FALSE) + 
  ggtitle("")
...
plot_info_tree$no_tree_species[plot_info_tree$no_tree_species == 0] = NA
p = ggplot(plot_info_tree)+
...
p
###################################################################
# Plot of the proportion of tree species represented by each area in the LC 
if('no_tree_accepted_wcvp_species' %in% names(plot_info_tree)){
  plot_info_tree$prop_tree_species = plot_info_tree$no_tree_species / plot_info_tree$no_tree_accepted_wcvp_species
  plot_info_tree$prop_tree_species[plot_info_tree$no_tree_accepted_wcvp_species == 0] = NA
  
  p = ggplot(plot_info_tree)+
    geom_sf(aes(fill=prop_tree_species),  col="transparent")+
    stat_sf_coordinates(aes(col=prop_tree_species)) + 
    scale_colour_distiller(direction=1, 
                           name="Percentage of\n tree Species\n in LC", na.value = 'black', labels = scales::label_percent()) +
    scale_fill_distiller(direction=1,
                         name="Percentage of\n tree Species\n in LC", na.value = 'black', labels = scales::label_percent())+
    coord_sf(expand=FALSE)+ 
    ggtitle("Percentage of Accepted tree Species \n in WCVP found in the LC for each region")
  
  if(separate_figure_folder){
    ggplot2::ggsave(plot = p, filename = paste0(figures_dir, '/','Map_proportion_of_tree_species_from_area_in_LC.pdf'),device = 'pdf', scale = 1, limitsize = FALSE)
  }
...
```{r, interactive map data setup_tree, eval = report_kind == 'interactive'}
# Due to the high level of detail in wgsrpd3 we need to go to a simplified geometry for the interactive geometry plots. 
# load all geo areas
wgsrpd3_level3_simp = BGSmartR::wgsrpd3_level3_simp
...
Note that not every region has trees. In particular, within WCVP there are `r sum(location_data$no_tree_accepted_wcvp_species == 0)` regions that have no trees. These regions are listed below:

<details>
<summary>Regions in WCVP with no trees</summary>
`r paste0(location_data$name[location_data$no_tree_accepted_wcvp_species == 0], collapse = ' <br>')`
</details>

 Therefore, those regions are removed prior to creating the maps.

`r if(report_kind == 'interactive'){"##### Maps of trees held in the living collection {.tabset}"}`
...
`r if(report_kind == 'interactive'){"###### By Number of Trees Held"}`
...
`r if(report_kind == 'interactive'){"###### By Proportion of Accepted Trees held"}`
...
`r if(report_kind == 'interactive'){"##### {-}"}`
 ...
 In the living collection `r round(sum(location_data$rep_tree)/sum(no_tree_accepted_species_wcvp > 0)*100,digits = 2)`% of regions with native trees are represented. The regions without representation are outlined below together with the number of trees in each region. 

```{r tree regions not in the LC - Table, eval = length(no_rep) > 0}
if(report_kind == 'static'){
  missing_regions |>
  flextable() |>
  set_caption(caption = "") |> 
...
Further information about trees from non-represented regions can be downloaded using the button below.

```{r link to download plants from missing tree regions, eval = report_kind == 'interactive' & length(no_rep) > 0 & do_download}
  wcvp_wanted |>
  download_this(
    output_name = "Trees (matched to WCVP accepted plants) from non-represented tree regions",
    output_extension = ".xlsx",
    button_label = "Download trees from non-represented regions with trees (xlsx)",
    button_type = "default",
    has_icon = TRUE,
    icon = "fa fa-save",
    class = "hvr-sweep-to-left",
