...
report = report[,match(c('AccNoFull','sanitised_taxon', 'TaxonNameFull', 'powo_name', 'extracted_author', 'POWO_taxon_name', 'POWO_taxon_authors', 'good_name', 'best_name', 'no_gardens', 'endemic', 'threatened', 'native', 'ItemStatusType', 'taxon_type', 'AccYear'), names(report))]
index_want = which(report$ItemStatusType == 'Existing' & !grepl('0|5|6',report$taxon_type))
no_want = length(index_want)
...
rare_global_list = interest[which(interest$no_gardens <= 10 | is.na(interest$no_gardens)),]
...
`r if(do_global){"Below we show the number of taxa in the LC against the number of sites the taxa is held globally. This is shown for all taxa as well as key collections - threatened, endemic, native. \n"}`
...
report_unique = interest[match(unique(interest$best_name), interest$best_name),]
...
globally_held_taxa = data_plot[,c(1,4,3,2)]
...
- **Very Rare**: found in between 2 and 10 collections.
...
Moreover, we do this for all taxa in the collection as well as the key collections (threatened, endemic and native). 
...
plot_data$type =  factor(plot_data$type, levels = c('All', 'Threatened', 'Endemic', 'Native'))
...
Top 20 most common taxa (removing cultivars and hybrids) in the collection
...
names(most_rare) = c('Taxa', "Is in BGCI's PlantSearch?")
 most_rare_table = DT::datatable(data = most_rare, rownames = FALSE, options = list(dom = 'tp',pageLength =  10))
 
 
most_common = data.frame(taxa_count[1:20])
names(most_common) = c('Taxa', 'Number of Individuals in the LC')
  most_common_table = DT::datatable(data = most_common, rownames = FALSE, options = list(dom = 'tp',pageLength =  5))

```

`r most_common_table`

In the LC there are `r no_unique +  no_unique_not_in_BGCI` taxa that are unique to the collection (either not contained (`r no_unique_not_in_BGCI`) or belongs to a single garden in BGCI's PlantSearch (`r no_unique`)) and only a single individual exists in the collection. There are shown in the table below. 

`r most_rare_table`

Further information (such as accession number, threatened status, etc) can be found in 'rare_global_list.xlsx'. management sheet. 
...
In this section we explore the duplication of taxa in the collection across both individuals and accessions. 
...
top_name = accession_taxa_group$best_name[which(accession_taxa_group$total == max(accession_taxa$total_accessions))][1]
...
top_name_threat = acc_threat$best_name[which(acc_threat$total == max(accession_taxa_threat$total_accessions))][1]
# top_name = interest$print_name[match(top_name,interest$best_name)]
...
top_name_endemic = acc_endemic$best_name[which(acc_endemic$total == max(accession_taxa_endemic$total_accessions))][1]
# top_name = interest$print_name[match(top_name,interest$best_name)]
...
top_name_nat = acc_nat$best_name[which(acc_nat$total == max(accession_taxa_nat$total_accessions))][1]
# top_name = interest$print_name[match(top_name,interest$best_name)]
...
acc_threat = item_taxa_group[item_taxa_group$threatened =='Threatened',]
item_taxa_threat = table(acc_threat$total)
item_taxa_threat = data.frame(total_items = as.numeric(names(item_taxa_threat)), 
                            no_species = as.numeric(item_taxa_threat))
top_name_threat = acc_threat$best_name[which(acc_threat$total == max(item_taxa_threat$total_items))][1]
# top_name = interest$print_name[match(top_name,interest$best_name)]

# Group the number of endemic taxa by the number of items they have 
acc_endemic = item_taxa_group[item_taxa_group$endemic =='Endemic',]
item_taxa_endemic = table(acc_endemic$total)
item_taxa_endemic = data.frame(total_items = as.numeric(names(item_taxa_endemic)), 
                            no_species = as.numeric(item_taxa_endemic))
top_name_endemic = acc_endemic$best_name[which(acc_endemic$total == max(item_taxa_endemic$total_items))][1]
# top_name = interest$print_name[match(top_name,interest$best_name)]

# Group the number of native taxa by the number of items they have 
acc_nat = item_taxa_group[item_taxa_group$native =='Native',]
item_taxa_nat = table(acc_nat$total)
item_taxa_nat = data.frame(total_items = as.numeric(names(item_taxa_nat)), 
                            no_species = as.numeric(item_taxa_nat))
top_name_nat = acc_nat$best_name[which(acc_nat$total == max(item_taxa_nat$total_items))][1]
# top_name = interest$print_name[match(top_name,interest$best_name)]
...
hor_stack_bar_plot = plot_ly(plot_data,
                             ...
                             orientation = 'h',
                            hovertext = ~hover, hoverinfo = 'text')
hor_stack_bar_plot_items <- hor_stack_bar_plot |> plotly::layout(xaxis = list(title = "Percentage of Taxa"),
                                   yaxis = list(title = ''),barmode = 'stack',hovermode = 'y unified')
```

Below we show a comparison of the number of accessions and items per taxa across key collections. 
...
Next we go into further detail across each key collection for individuals and accessions separately.
...
- The mean number of accessions per taxa is **`r round(mean(accession_taxa_group$total),digits=2)`**.
- The median number of accessions per taxa is **`r median(accession_taxa_group$total)`**.
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_threat$total_accessions)`** this occurs for the accession **`r top_name_threat`**. 
- The mean number of accessions per taxa is **`r round(mean(acc_threat$total),digits=2)`**.
- The median number of accessions per taxa is **`r median(acc_threat$total)`**.
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_endemic$total_accessions)`** this occurs for the accession **`r top_name_endemic`**. 
- The mean number of accessions per taxa is **`r round(mean(acc_endemic$total),digits=2)`**.
- The median number of accessions per taxa is **`r median(acc_endemic$total)`**.
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_nat$total_accessions)`** this occurs for the accession **`r top_name_nat`**. 
- The mean number of accessions per taxa is **`r round(mean(acc_nat$total),digits=2)`**.
- The median number of accessions per taxa is **`r median(acc_nat$total)`**.
...
- The maximum number of items for a given taxon is **`r max(item_taxa$total_items)`** this occurs for the item **`r top_name`**. 
- The mean number of accessions per taxa is **`r round(mean(item_taxa_group$total),digits=2)`**.
- The median number of accessions per taxa is **`r median(item_taxa_group$total)`**.
- **`r format(sum(item_taxa_group$total == 1), big.mark =',')`** taxa have a single accession.
...
- The maximum number of items for a given taxon is **`r max(item_taxa_threat$total_items)`** this occurs for the item **`r top_name_threat`**. 
- The mean number of items per taxa is **`r round(mean(acc_threat$total),digits=2)`**.
- The median number of items per taxa is **`r median(acc_threat$total)`**.
- **`r format(sum(acc_threat$total == 1), big.mark =',')`** taxa have a single item.
...
- The maximum number of items for a given taxon is **`r max(item_taxa_endemic$total_items)`** this occurs for the item **`r top_name_endemic`**. 
- The mean number of items per taxa is **`r round(mean(acc_endemic$total),digits=2)`**.
- The median number of items per taxa is **`r median(acc_endemic$total)`**.
- **`r format(sum(acc_endemic$total == 1), big.mark =',')`** taxa have a single item.
...
- The maximum number of items for a given taxon is **`r max(item_taxa_nat$total_items)`** this occurs for the item **`r top_name_nat`**. 
- The mean number of items per taxa is **`r round(mean(acc_nat$total),digits=2)`**.
- The median number of items per taxa is **`r median(acc_nat$total)`**.
- **`r format(sum(acc_nat$total == 1), big.mark =',')`** taxa have a single item.
...
We can view accession size in two ways, either the number of individuals in an accession at the time of accessioning or the current number of individuals existing in an accession (i.e including individual death/removal). We shall use both approaches. 

We first produce a summary bar chart showing how the number of individuals per accession varies across the key collections. 
...
plot_data$type = stringr::str_to_title(plot_data$type)
plot_data$type =  factor(plot_data$type, levels = c('All', 'Threatened', 'Endemic', 'Native'))
plot_data$hover = paste0(plot_data$percent, '% (',plot_data$no_species,') of accessions have ',plot_data$total_items,' individual/s.' )
hor_stack_bar_plot = plot_ly(plot_data,
                             type = 'bar',
                             y = ~type,
                             x = ~percent,
                             ...
                             orientation = 'h',
                            hovertext = ~hover, hoverinfo = 'text')
hor_stack_bar_plot_items <- hor_stack_bar_plot |> plotly::layout(xaxis = list(title = "Percentage of Accessions"),
                                   yaxis = list(title = ''),barmode = 'stack',hovermode = 'y unified')
```

```{r, number of items per accessions at time of accessioning - data}
# Reduced to only unique items.
...
plot_data$type = stringr::str_to_title(plot_data$type)
plot_data$type =  factor(plot_data$type, levels = c('All', 'Threatened', 'Endemic', 'Native'))
plot_data$hover = paste0(plot_data$percent, '% (',plot_data$no_species,') of accessions have ',plot_data$total_individals,' individual/s.' )
hor_stack_bar_plot = plot_ly(plot_data,
                             type = 'bar',
                             y = ~type,
                             x = ~percent,
...
`r if(report_kind == 'interactive'){"###### Exisiting individuals only"}`
```{r Number of items per accessions summary  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
hor_stack_bar_plot_items
```

`r if(report_kind == 'interactive'){"###### Existing accessions (At time of accessioning)"}`
```{r Number of items per accessions summary timeof accessioning  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
hor_stack_bar_plot_items_acc_time
```

`r if(report_kind == 'interactive'){"##### {-}"}`


In the following sections we look in further detail of the number of individuals per accession and how this has varied over time across the whole collection as well as key collections.

***

#### Individuals per accession (current) 
 
 In this section we explore the number of individuals per accession that are currently existing in the collection.
 
`r if(report_kind == 'interactive'){"#####   {.tabset}"}`
`r if(report_kind == 'interactive'){"###### All"}`

```{r Number of individuals per accession  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_items$all
```

Where 

- The maximum number of individuals for a given accession is **`r max(item_taxa$total_items)`** this occurs for the accession **`r top_acc`** ( **`r top_name`** ). 
- The mean number of individuals per accession is **`r round(mean(item_taxa_group$total),digits=2)`**.
- The median number of individuals per accession is **`r median(item_taxa_group$total)`**.
- **`r format(sum(item_taxa_group$total == 1), big.mark =',')`** accessions have a single individual.



`r if(report_kind == 'interactive'){"###### Threatened"}`
```{r Number of accessions per threatened accession  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_items$threatened
```

Where 

- The maximum number of individuals for a given accession is **`r max(item_taxa_threat$total_items)`** this occurs for the accession **`r top_acc_threat`** (**`r top_name_threat`**). 
- The mean number of individuals per accession is **`r round(mean(item_threat$total),digits=2)`**.
- The median number of individuals per accession is **`r median(item_threat$total)`**.
- **`r format(sum(item_threat$total == 1), big.mark =',')`** accessions have a single individual.


`r if(report_kind == 'interactive'){"###### Endemic"}`
```{r Number of accessions per endemic accession  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_items$endemic
```

Where 

- The maximum number of individuals for a given accession is **`r max(item_taxa_endemic$total_items)`** this occurs for the accession **`r top_acc_endemic`** (**`r top_name_endemic`**). 
- The mean number of individuals per accession is **`r round(mean(item_endemic$total),digits=2)`**.
- The median number of individuals per accession is **`r median(item_endemic$total)`**.
- **`r format(sum(item_endemic$total == 1), big.mark =',')`** accessions have a single individual.


`r if(report_kind == 'interactive'){"###### Native"}`
```{r Number of accessions per native accession  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_items$native
```

Where 

- The maximum number of individuals for a given accession is **`r max(item_taxa_nat$total_items)`** this occurs for the accession **`r top_acc_nat`** (**`r top_name_nat`**). 
- The mean number of individuals per accession is **`r round(mean(item_nat$total),digits=2)`**.
- The median number of individuals per accession is **`r median(item_nat$total)`**.
- **`r format(sum(item_nat$total == 1), big.mark =',')`** accessions have a single individual.
...
#### Individuals per accession (at time of accessioning) 
 
  In this section we explore the number of individuals per accession at the time of accessioning (for existing accessions in the collection).

`r if(report_kind == 'interactive'){"#####   {.tabset}"}`
`r if(report_kind == 'interactive'){"###### All"}`

```{r Number of individuals per accession at time of accessioning  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_acc$all
```

Where 

- The maximum number of individuals for a given accession is **`r max(individal_taxa$total_individal)`** this occurs for the accession **`r a_top_acc`** ( **`r a_top_name`** ). 
- The mean number of individuals per accession is **`r round(mean(individal_taxa_group$total),digits=2)`**.
- The median number of individuals per accession is **`r median(individal_taxa_group$total)`**.
- **`r format(sum(individal_taxa_group$total == 1), big.mark =',')`** accessions have a single individual.



`r if(report_kind == 'interactive'){"###### Threatened"}`
```{r Number of accessions per threatened accession at time of accessioning  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_acc$threatened
```

Where 

- The maximum number of individuals for a given accession is **`r max(individal_taxa_threat$total_individals)`** this occurs for the accession **`r a_top_acc_threat`** (**`r a_top_name_threat`**). 
- The mean number of individuals per accession is **`r round(mean(acc_threat$total),digits=2)`**.
- The median number of individuals per accession is **`r median(acc_threat$total)`**.
- **`r format(sum(acc_threat$total == 1), big.mark =',')`** accessions have a single individual.


`r if(report_kind == 'interactive'){"###### Endemic"}`
```{r Number of accessions per endemic accession at time of accessioning  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_acc$endemic
```

Where 

- The maximum number of individuals for a given accession is **`r max(individal_taxa_endemic$total_individals)`** this occurs for the accession **`r a_top_acc_endemic`** (**`r a_top_name_endemic`**). 
- The mean number of individuals per accession is **`r round(mean(acc_endemic$total),digits=2)`**.
- The median number of individuals per accession is **`r median(acc_endemic$total)`**.
- **`r format(sum(acc_endemic$total == 1), big.mark =',')`** accessions have a single individual.


`r if(report_kind == 'interactive'){"###### Native"}`
```{r Number of accessions per native accession at time of accessioning  - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
figs_acc$native
```

Where 

- The maximum number of individuals for a given accession is **`r max(individal_taxa_nat$total_individals)`** this occurs for the accession **`r a_top_acc_nat`** (**`r a_top_name_nat`**). 
- The mean number of individuals per accession is **`r round(mean(acc_nat$total),digits=2)`**.
- The median number of individuals per accession is **`r median(acc_nat$total)`**.
- **`r format(sum(acc_nat$total == 1), big.mark =',')`** accessions have a single individual.
...
#### How the number of individuals per accession has varied over time

 In this section we explore how the number of individuals per accession at the time of accessioning has varied across time.
...
columns = c('all','threatened','endemic', 'native')
