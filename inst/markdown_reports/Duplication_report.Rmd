...
- Global level: how many collections globally are taxa in the living collection found.
...
This reduces the number of records in the living collection to `r format(no_records, big.mark =',')` from `r format(nrow(interest), big.mark =',')`.

Moreover, where possible we will use the taxonomic name from matching to POWO online, if this is not possible we use the original name from the living collection. 
...
`r if(do_global & no_different_rare_taxa > 0){paste0("Note that there are ", no_different_rare_taxa, " taxa in the living collection classed as very rare (<= 10 living collections globally) where the original taxonomic name from the report and POWO's taxonomic name differ. See downloadable data at the end of this section.")}`

`r if(do_global){"Below we show the number of taxa in the living collection against the number of sites the taxa is held globally. This is shown for all taxa as well as key collections - threatened, endemic, native. \n"}`
...
Rarity of species at the living collection
...
In the living collection there are `r no_unique +  no_unique_not_in_BGCI` taxa that are unique to the collection (either not contained (`r no_unique_not_in_BGCI`) or belongs to a single garden in BGCI's PlantSearch (`r no_unique`)) and only a single item exists in the collection. There are shown in the table below. 
...
In this section we explore the duplication of taxa in the collection across both items and accessions. 
...
Next we go into further detail across each key collection for items and accessions separately.
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa$total_accessions)`** this occurs for the accession **`r top_name_acc`**. 
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_threat$total_accessions)`** this occurs for the accession **`r top_name_threat_acc`**. 
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_endemic$total_accessions)`** this occurs for the accession **`r top_name_endemic_acc`**. 
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_nat$total_accessions)`** this occurs for the accession **`r top_name_nat_acc`**. 
...
- The maximum number of accessions for a given taxon is **`r max(accession_taxa_tree$total_accessions)`** this occurs for the accession **`r top_name_tree_acc`**. 
- The mean number of accessions per taxa is **`r round(mean(acc_tree$total),digits=2)`**.
- The median number of accessions per taxa is **`r median(acc_tree$total)`**.
...
- The maximum number of items for a given taxon is **`r max(item_taxa_tree$total_items)`** this occurs for the item **`r top_name_tree`**. 
- The mean number of items per taxa is **`r round(mean(acc_tree$total),digits=2)`**.
- The median number of items per taxa is **`r median(acc_tree$total)`**.
