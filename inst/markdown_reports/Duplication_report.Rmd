...
if(length(unique_records$best_name) == 0){in_BGCI = character(0)}else{in_BGCI = 'True'}
unique_sum = data.frame(name = unique_records$best_name, in_BGCI = in_BGCI)
if(length(unique_not_in_BGCI$best_name) == 0){in_BGCI = character(0)}else{in_BGCI = 'False'}
unique_not_in_sum = data.frame(name = unique_not_in_BGCI$best_name, in_BGCI = in_BGCI)
...
most_common = data.frame(taxa_count[1:min(20, length(taxa_count))])
...
```{r, download button for rare global list}
rare_global_list_format |>
  downloadthis::download_this(
    output_name = "rare_global_list",
    output_extension = ".xlsx",
...
threat_simp[nrow(threat_simp)+1,] = c(20, threat_big_20)
  ...
  threat_simp = data.frame(A=1, B=1) ; names(threat_simp) = c('Accessions', 'Taxonomic Names')
  threat_simp = threat_simp[-1,]
...
if(nrow(endemic)> 0){
  ...
  endemic_simp = data.frame(A=1, B=1) ; names(endemic_simp) = c('Accessions', 'Taxonomic Names')
  endemic_simp = endemic_simp[-1,]
}
...
plot_data = lapply(1:length(accession_taxa_all_type), function(index){
  d = accession_taxa_all_type[[index]]
  ...
  to_remove = which(data_supp$no_species == 0 | is.na(data_supp$no_species))
  if(length(to_remove) > 0) data_supp = data_supp[-to_remove,]
  data_supp$percent = round(as.numeric(data_supp$no_species)/sum(as.numeric(data_supp$no_species))*100,digits=2)
  if(nrow(data_supp) == 0){
    return(NULL)
  } 
  data_supp$type = names(accession_taxa_all_type)[index]
  data_supp
})
...
## Create horizontal stacked bar for key collection and all in one plot. 
plot_data = lapply(1:length(item_taxa_all_type), function(index){
  d = item_taxa_all_type[[index]]
...
## Create horizontal stacked bar for key collection and all in one plot. 
plot_data = lapply(1:length(individal_taxa_all_type), function(index){
  d = individal_taxa_all_type[[index]]
