...
# #Inputs
# load('/Users/jakepowell/Cambridge/Enriched_reports_Jake/BuenosAiresCT_enriched_report.rda')
# collection = 'Benmore'
# 
...
- Global level: investigates how many collections globally taxa from the LC are found in.
...
Doughnut/Pie graph of rarity of species at the LC
...
- The maximum number of accessions for a given taxa is **`r max(accession_taxa$total_accessions)`** this occurs for the accession **`r taxa_no_accessions$taxa[taxa_no_accessions$no_accessions == max(accession_taxa$total_accessions)][1]`**. 
...
Doughnut/Pie chart of number of accessions per taxa. 
...
- The maximum number of items for a given taxa is **`r max(item_taxa$total_items)`**. An example is the taxa **`r data_cur$taxa[data_cur$no_items == max(item_taxa$total_items)][1]`**. 
...
Doughnut/Pie chart of number of items per taxa. 
...
For this section we consider all accessions that have at least one item still existing. It can be tricky here to determine what number of items per accession means since for all the existing plants in the LC some items from an accession might have died. Therefore, we will produce two versions of each of the following figures, one where we take into account the number of items for each accession existing in the LC at this moment in time and another where we take the number of items in each accession at the time when the accession occurred. 

#### Only considering items currently existing in the LC

Bar graph of the number of accessions in the collection by their number of items (existing at this moment).
...
- The maximum number of items in an accession is **`r max(item_dist_existing$total_items)`**, for example the accession **`r max_accession[1]`** (`r plant_max_accession_existing[1]`). 
...
Bar graph of the number of accessions in the collection by their number of items (at the time of accession).
...
- The maximum number of items in an accession is **`r max(item_dist$total_items)`** for example the accession **`r max_accession[1]`** (`r plant_max_accession[1]`). 
