...
In this section, we are interested in trends in the collection size by exploring the change in items, accessions, taxa, species, genera and families. Taxa is defined by the combination of taxonomic name and authority such as *Androsace sarmentosa* subsp. *primuloides* (Hook.f.) Govaerts. Species is defined by the genus and species pair such as *Androsace sarmentosa*.
...
`r if(FALSE){paste0("Biological: ", tolower(data_type), "that are either a genus and species pair, or contains subspecies, varities and forma. Excluding records that are horticultural.")}`
...
- Indeterminate, records such as *Hydnophytum* sp. or Indet. *caryophyllaceae*. 

Note that the groups above have being defined such that all `r tolower(data_type)` cannot belong to multiple categories. 
...
`r if(report_kind == 'interactive'){"#####  Category Trends {.tabset}"}`
...
In this section we explore the trends in provenance. Each accession in the garden has one of the following provenance types:
...
In this section we look at trends across the key collections in the LC, this includes:
...
int_tree_plots = group_trend_plots(years, no_tree, "Tree", report_kind = report_kind, separate_figure_folder = separate_figure_folder, data_type = data_type)
