...
In this section we explore the number of native plants in the living collection compared to all plants in the living collection. In particular, we will determine if the number of native plants in the living collection is stable, increasing or decreasing. 

We will examine the proportion of the living collection that is native, this is shown in the table below.
...
`r if(do_over_time){"In this section we explore how the number of native species in the living collection has varied over time. For each year we use the accession year and item status date to determine which items in the collection are existing on the 31st of December each year."}`
...
`r if(report_kind == 'interactive' & do_over_time){"##### Number of native plants over time {.tabset}"}`
...
`r if(do_over_time){"Next we show native plants in the living collection as a percentage of all plants in the living collection over time."}`
...
In the living collection there are `r format(no_native_items_LC, big.mark=',')` items corresponding to  `r format(no_native_accessions_LC, big.mark=',')` accessions of existing native plants. In the following table we show how many families/genus/species/taxa of native plants are represented in the living collection (compared to all plants from `r paste0(location_name, collapse = ', ')` in WCVP). Note that here species is defined as the genus species pair and taxa is the taxonomic name and author (thus including subspecies, variety, forma).
...
`r if(do_over_time){"Below we produce figures detailing how the representation of native families/genus/species/taxa has changed over time in the living collection."}`
...
`r if(do_over_time){"We will now look at how the provenance of native plants in the living collection has changed over time."}`
...
#### Red list categories of threatened native taxa
...
### Proportion of native taxonomy found in the living collection

In this section we create sunburst and treemap charts showing all of the genera native to `r paste0(location_name, collapse = ', ')` within WCVP, where the genera are grouped into families, order and higher level lineages. The charts are shaded lighter to darker orange depending on the proportion of the native lineage is found in the collection. 
