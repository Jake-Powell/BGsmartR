...
# extinct = TRUE
# doubtful_locations = FALSE
# output_dir =  '/Users/jakepowell/Desktop/module_native_example'
```
...
`r if(coord_contained){paste0("Within TDWG geographical codes ",collection," is  found within the region ",coord_name,".")}`
`r if(!coord_contained){paste0("Within TDWG geographical codes ",collection," is not found within any regions, however the nearest region is ",coord_name,".")}`

`r if(is.na(bru_codes[1])){paste0("Therefore, any taxa whose geographic distribution includes ", location_name, " is classed as native to the LC. Otherwise, taxa are classed as non-native.")}`

`r if(is.na(bru_codes[1])){paste0("The location of the LC and the native region are shown below.")}`
...
`r if(!is.na(bru_codes[1])){paste0("The native region, and LC location (if within the native region) are shown below.")}`
...
names(df) = c('Category', 'Native', ' Number found in LC', 'Proportion in LC')
...
if(length(family_not_in) > 0){
  ...
  wcvp_wanted_format$In_col = any_in_collection
    names(wcvp_wanted_format) = c('Family', 'Total Native Taxa', 'Family in LC (not-native)')
...
In the LC there are `r format(no_native_items_LC, big.mark=',')` items corresponding to  `r format(no_native_accessions_LC, big.mark=',')` accessions of existing native plants. In the following table we show how many families/genus/species/taxa of native plants are represented in the LC (compared to all plants from `r paste0(location_name, collapse = ', ')` in WCVP). Note that here species is defined as the genus species pair and taxa is the taxonomic name and author (thus including subspecies, variety, forma).
...
`r if(length(family_not_in) > 0){"The native families missing from the collection are outlined below, together with the total number of native taxa in each family."}`
...
`r if(length(family_not_in) > 0){"Further information about the plants from the missing families can be downloaded using the button below."}`
...
wcvp_wanted |>
  download_this(
    output_name = "WCVP native plants not in the LC",
    output_extension = ".xlsx",
    button_label = "Download native families not in the LC (xlsx)",
    button_type = "default",
    has_icon = TRUE,
    icon = "fa fa-save",
...
# type = 'Family' ;whole_collection = df; data = over_time_info ; title = 'families'
represented_figure <- function(data,whole_collection, type, title){
  count = as.numeric(unlist(over_time_info[match(type, names(over_time_info))]))
...
fig = represented_figure(data = over_time_info, whole_collection = df, type = 'Family', title = 'families')
...
fig = represented_figure(data = over_time_info, whole_collection = df, type = 'Genus', title = 'genera')
...
fig = represented_figure(data = over_time_info, whole_collection = df, type = 'Species', title = 'species')
...
fig = represented_figure(data = over_time_info, whole_collection = df, type = 'Taxa', title = 'taxa')
...
`r if(report_kind == 'interactive' & do_over_time){"#####  Proportion of the collection that is native  {.tabset}"}`
...
native_data = native_data ; LC_data = LC_data ; type = 'Items' ; title = 'items'
...
### What is the provenance of native plants?
...
names(flex_data) = c('Provenance', 'Native items', '% of items in LC', 'Native accessions', '% of accessions in LC')
...
<!-- ```{r  Provenance of existing native accessions in the LC - interactive, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'} -->
...
fig <- plot_ly(data_wanted, x = ~year, y = ~prop, color = ~Provenance, type = 'bar', text = ~text,
               hovertemplate = '%{text}',
               sort = FALSE, colors = interactive_colour(4))
fig <- fig |> layout(yaxis = list(title = 'Percentage',ticksuffix  ='%', hoverformat = '.1f' ),
                     xaxis = list(title = "Year", hoverformat = 'Year: '),
                     legend=list(title=list(text='')),
                     barmode = 'stack')
...
### What is the turnover of native plants?

This section looks at the turnover (gain/loss) of accessions of native plants in the LC.
...
### How long do native plants survive? 
...
enriched_report_wanted = enriched_report[as.numeric(enriched_report$AccYearOrig) > 1650 & as.numeric(enriched_report$AccYearOrig) < as.numeric(format(Sys.Date(),'%Y')),]
...
if(report_kind == 'interactive'){
  # figgo = plot_ly(data = survival_data_plot, x =~p.data.time, y = ~p.data.estimate, color = ~p.data.strata, mode = 'lines', type = 'scatter', colors = interactive_colour(2))
  # figgo = figgo |> layout(legend = list(orientation = 'h'),
  #                         yaxis = list(title = paste0("Overall Survival Probability")),
...
### How many of native plants are endemic or threatened?
...
LC_native_exisiting <<- LC_native_exisiting
...
`r if(need_venn){paste0("In this section, we concentrate on native plants in the LC that are either endemic to ", paste0(location_name, collapse = ', ') ," or on IUCN's Red List.")}`

`r if(overlap & need_venn){paste0("Withn the native sub-collection of ", collection, " there are ", total_endemic, " endemic items and ", total_threatened, " threatened items, where ", total_threat_endemic, " are both. Therefore, ",round(total_endemic/nrow(LC_native_exisiting)*100,digits = 2), "% of the native items are endemic and ",round(total_threatened/nrow(LC_native_exisiting)*100,digits = 2),"% are threatened.")}`
...
if(report_kind == 'interactive'){
    DT::datatable(data_compressed,rownames = F,
                  # caption = 'Endemic and threatened plants in the native sub-collection',
                  options = list(dom = 'tp',pageLength =  5), filter = list(position = "top"))
...
data_save = data
data_save = data_save[data_save$endemic | data_save$threatened,]
...
### What are the Red List categories of threatened native plants and how has this varied over time?
...
### What proportion of native taxonomy is in the LC?

In this section we create sunburst and treemap charts showing all of the genera native to `r paste0(location_name, collapse = ', ')` within WCVP, where the genera are grouped into families, order and higher level lineages. The charts are shaded between white and black depending on the proportion of the native lineage is found in the collection. 
...
`r if(report_kind == 'interactive'){"#####  Native genera in/not in the LC {.tabset}"}`
