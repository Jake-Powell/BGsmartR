...
```{r, check if we have any native plants, warning=NA}
no_native_plants = nrow(LC_native) == 0
if(no_native_plants){
  warning('There are no native plants!')
}
```

`r if(no_native_plants){"There are no native plants."}`
`r if(no_native_plants){knitr::knit_exit()}`

```{r, How many native species are in the LC - data part 2}
