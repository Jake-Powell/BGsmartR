...
if(any(is.na(report$ProvenanceCode))) report$ProvenanceCode[is.na(report$ProvenanceCode)] = 'NA'
...
funnel_data_format = data.frame(stage = funnel_data$names[-1],
                                stage_start = funnel_data$values[-8] |> as.numeric(),
                                Yes = funnel_data$stage_number[-1]|> as.numeric())
funnel_data_format$No = funnel_data_format$stage_start - funnel_data_format$Yes 
funnel_data_format = funnel_data_format[,-2]
funnel_data_format = funnel_data_format[-6,]
funnel_data_format$stage = c('1) Is it a cultivar/indeterminate?',
                             '2) Does it match taxonomic name and authority?',
                             '3) Does it match taxonic name and part of the authority?',
                             '4) Does only the taxonomic name match?',
                             '5) Can we fix the taxonomic name to find a match?',
                             '6) No match'
                             )

dd = reshape(funnel_data_format, idvar = "stage", varying = list(2:ncol(funnel_data_format)),
             v.names = "count", timevar = "kind", times = names(funnel_data_format)[-1], direction = "long")
dd$stage_wrap = dd$stage |> stringr::str_wrap(width = 30)
dd$stage_wrap = factor(dd$stage_wrap, levels = unique(dd$stage_wrap) |> rev())
dd$hover = paste0(dd$stage, ' <br> ', dd$kind, ' <br> ', dd$count, ' taxa')

hor_stack_bar_plot = plotly::plot_ly(dd,
                             type = 'bar',
                             y = ~stage_wrap,
                             x = ~count,
                             color = ~kind,
                             colors = interactive_colour(2) |> rev(),
                             orientation = 'h',
                             hovertext = ~hover, hoverinfo = 'text',
                             showlegend = TRUE
                             # marker = list(color = ~color)
                             )|>
  plotly::layout(xaxis = list(title = "Taxonomic Names"),
                 yaxis = list(title = 'Stages'),barmode = 'stack'
                 # hovermode = 'y unified'
  )
...
comb$color[which(comb$category == 'Tree')]   = color_binary[2] 
comb$color[which(comb$category == 'Not tree')]   = color_binary[1]    
...
if(has_redlist_cat){
  if(report$IUCNRedList |> table() |> length() == 0){has_redlist_cat = F} # If column exists and all values are missing (i.e NA) set to the column not existing
}
...
if(!has_redlist_cat){
  ...
  for_plot = new
  if(ncol(for_plot) == 1){for_plot$B = character(0)}
...
max_year = Sys.Date() |> format('%Y') |> as.numeric()
...
year = unique(c(last_update_acc$year,last_update_items$year )) |> sort()
no_acc = rep(0,length(year)) ; no_acc[match(last_update_acc$year, year)] = last_update_acc$no_acc
acc_cumulative = rep(0,length(year)) ; acc_cumulative[match(last_update_acc$year, year)] = last_update_acc$cumulative
table_data = data.frame(year = year,
                        accessions = no_acc,
                        acc_cum = acc_cumulative,
...
interest[is.na(interest)] = 'NA'
unique_prov = unique(interest)
prov_issue = interest[!interest %in% c("Unknown", "Garden", "Wild", "Wild-derived")]

prov_issue_flag = ifelse(length(prov_issue)>0, TRUE,FALSE)

if(prov_issue_flag){
  table_prov_issue = prov_issue |> table() |> data.frame()
...
interest$ProvenanceCode[is.na(interest$ProvenanceCode)] = 'Unknown'
interest_existing = interest[interest$ItemStatusType == 'Existing',]

no_unknown = sum(interest_existing$ProvenanceCode == 'Unknown',na.rm = T)
