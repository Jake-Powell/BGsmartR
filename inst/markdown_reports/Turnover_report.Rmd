...
names(trends) = names(existing_each_year)
```
...
ay <- list(
  ...
  overlaying = "y",
  side = "right",
  title = paste0('Total in LC'))

fig <- fig %>% add_trace(x = years, y = trend, name = "Total", yaxis = "y2", mode = "lines+markers", type = "scatter", line = list(color = 'black'), marker = list(color = 'black'))

# Set figure title, x and y-axes titles
fig <- fig %>% layout(
  title = "", yaxis2 = ay,
  xaxis = list(title="Year"),
  yaxis = list(title=paste0("Turnover"))
)%>%
...
fig <- fig |> layout(yaxis = list(title = paste0("Percentage")),
                   xaxis = list(title = "Year"))
fig <- fig |> add_trace(x = ~year, y = ~prop_specific_collection, type = 'scatter', mode = 'markers', name = paste0('LC'), marker = list(color = 'black'), hovertemplate = paste("Collection: %{y:.2f}%<extra></extra>"))
fig <- fig |> layout(hovermode = 'x unified')
...
fig <- fig |> layout(yaxis = list(title = paste0("Percentage")),
                   xaxis = list(title = "Year"))
fig <- fig |> add_trace(x = ~year, y = ~prop_specific_collection, type = 'scatter', mode = 'markers', name = paste0('LC'), marker = list(color = 'black'))
fig <- fig |> layout(hovermode = 'x unified')
...
This section looks at the provenance of gained and lost `r tolower(data_type)` to the collection. The provenance of an `r tolower(data_type)` can be in one of four groups, namely: Wild, Wild-derived, Garden or Unknown.
...
loss_all = data.frame(year = exporting_data_store$turnover_provenance_unknown$Year,
 ...
 ff$group = factor(ff$group, levels = unique(ff$group))
 ...
 ff_prop$group = factor(ff_prop$group, levels = unique(ff_prop$group))
...
This section looks at the turnover of plants that are native to the location of `r collection` In other words plants that are native to `r location_name`. Here we use [Plants of the World Online (POWO)](https://powo.science.kew.org) to determine the geography distribution of individual plants, where we only consider regions where the plant is `r location_type_text`
...
This section looks at the turnover of plants that are endemic, this means plants that are native to a single geography region (not only native plants). Here we use [Plants of the World Online (POWO)](https://powo.science.kew.org) to determine the geography distribution of individual plants, where we only consider regions where the plant is `r location_type_text`
...
This section looks at the turnover of plants that are trees. Here we use [BGCI's GlobalTreeSearch](https://tools.bgci.org/global_tree_search.php) combined with [Plants of the World Online (POWO)](https://powo.science.kew.org) (to look at synonymy) to determine which plants in the LC are trees. Note that we use the Genus and Species base to determine trees therefore, this will also include cultivated trees if any exist.
...
`r if(report_kind == 'interactive' & do_type){"#####  Proportional Turnover of tree species {.tabset}"}`
