---
title: "Overview of key taxa in the LC"
...
This report explores key taxa in `r collection`, and how the representation of key taxa has varied over time.
...
We explore two types of key taxa, namely:
...
A taxa in the collection is classed as **endemic** if it is native to a single BRU level 3 area. Note that there area does not have to be the one the collection belongs to. Therefore, we need geographic information to quantify if a taxa is endemic. We extract geographic information using [Plants of the World Online](https://powo.science.kew.org) (POWO).
...
where both species and taxa only count records that are found in WCVP. 
...
if(!is.null(is.null(endemic_species_per_region))){
...
```{r, interactive map data setup, eval = report_kind == 'interactive'}
  ...
  colos = ramp(seq(0, 1, length = 99999))
  
  
  index = seq(0,1,0.00001)
...
# 
#   
#   plot_int_info$no_endemic_species[plot_int_info$no_endemic_species == 0] = -1
#   fig = plot_ly(plot_int_info,  text = ~hover, hoverinfo = 'text') |>
#     add_trace(type="choroplethmapbox",
#      geojson=data,
#      locations=locations$code,
#      z=plot_int_info$no_endemic_species+1,
#      zmin = 0, zmax = max(plot_int_info$no_endemic_species),
...
#  
#   fillcolor = unlist(lapply(col_scale[plot_int_info$no_endemic_species+1/max(plot_int_info$no_endemic_species+1)], function(x){x[[2]]}))
#   fig = fig |>  add_trace(type="scattermapbox",
#                           mode = 'markers',
#                           data = center_df,
#                           lon = center_df$long,
...
```{r, Provenance of endemic plants in the LC pie charts, echo = FALSE, fig.fullwidth=TRUE, fig.dim = c(10, 6), eval = report_kind == 'static'}
# Basic piechart
