...
if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Bar_how_many_elements.html'))
}
...
if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Bar:how_many_each_taxonomic_unit.html'))
}
...
fig <- plot_ly(taxa_infra, labels = ~InfragenericLevel, values = ~number, type = 'pie')

if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Pie_how_many_species_hort_infra.html'))
}
fig
```
where:
...
if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Pie_provenance.html'))
}
...
if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Geography.html'))
}
```
...
if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Pie_endemic.html'))
}
...
#### Number and/or proportion of threatened species?
...
fig <- plot_ly(threat, labels = ~threat, values = ~number, type = 'pie')

if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Pie_threatened.html'))
}
...
#### Number and/or proportion of different threatened species types?

```{r, echo=F, fig.fullwidth=TRUE, fig.dim = c(10, 4)}
########################################
### threatened category pie chart.
########################################
# First get POWO taxon name if it exists, if not use sanitised name.
red_table = table(report$redList_category)
...
fig <- plot_ly(threat_type, labels = ~threat, values = ~number, type = 'pie')

if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Pie_threatened_categories.html'))
}
...
fig <- plot_ly(extant_gardens, x = ~extant, y = ~in_collection, 
...
fig = manipulateWidget::combineWidgets(list = list(fig= fig, fig2= fig2), ncol = 2)

if(save_widgets){
  htmlwidgets::saveWidget(fig, file = paste0('Widgets/','Rareity_extant.html'))
}
...
xlsx::addDataFrame(basics, sh1, startRow = 1,row.names = F)
sh2 = xlsx::createSheet(wb, 'How many of each taxonomic units')
xlsx::addDataFrame(taxa, sh2, startRow = 1,row.names = F)
sh3 = xlsx::createSheet(wb, 'Species, Horticultral, infraspecific')
xlsx::addDataFrame(taxa_infra, sh3, startRow = 1,row.names = F)
...
xlsx::saveWorkbook(wb, file = paste0(collection,'_data_for_figures.xlsx'))
```
