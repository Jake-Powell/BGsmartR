...
fig <- plot_ly(taxa, x = ~TaxonomicCategory, y = ~count, type = 'bar', marker = list(color = ~colors))
fig <- fig %>% layout(title = "",
...
index = seq(0,1,0.01)
colours = c("#FFFFFF",rev(viridis::plasma(100)))
...
For plants that we successfully matched to POWO (**`r format(nrow(re),big.mark=',')`** items corresponding to **`r format(length(unique(re$good_name)),big.mark=',')`** taxa) we have geographic distribution data. 
...
- Name of the `Region`.
- `Number of items` in the collection that are native to the region.
- `Number of Taxa` in the collection that are native to the region.
...
species = report$good_name[which(!grepl('0|5|6',report$infrageneric_level))]
      ...
      report_cur = report_cur[which(!grepl('0|5|6',report_cur$infrageneric_level))[wanted_index],]
...
fig1 <- plot_ly(threat, labels = ~threat, values = ~number,
               ...
               hole =.4) %>% layout(title = 'All records')
...
fig2 <- plot_ly(threat_type, labels = ~threat, values = ~number,
               type = 'pie',
               marker = list(colors = colors),
               pull = rep(0,nrow(threat_type)),
               hole =.4) %>% layout(title = "'Threatened' Breakdown")
...
Where threatened species are taken to be those that are either: Vulnerable, Endangered, Critically Endangered, Extinct in the Wild or Extinct. 
...
plants = unlist(lapply(extant_numbers, function(x){
  return(paste0(report_unique$good_name[which(report_unique$no_gardens == x)], collapse = '<br />'))
}))
...
fig2 <- plot_ly(grouped_extant, labels = ~group, values = ~number,
               type = 'pie',
               marker = list(colors = colors),
...
Of the records we were able to match to POWO the sunburst chart below represents the taxonomic diversity of your collection. 
...
From the table above, we see that the collection has **`r grow_shrink`**
