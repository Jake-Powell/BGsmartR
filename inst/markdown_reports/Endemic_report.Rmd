...
```{r, functions for turnover old, eval = F}
...
A taxa in the collection is classed as **endemic** if it is native to a single BRU level 3 area. Note that the area does not have to be the one the collection belongs to.

<!-- A taxa in the collection is classed as **endemic** if it is native to a single BRU level 3 area. Note that the area does not have to be the one the collection belongs to. Therefore, we need geographic information to quantify if a taxa is endemic.  To determine the geographic distribution of items in the living collection we enrich with information from [Plants of the World Online](https://powo.science.kew.org), by matching taxonomic names. POWO uses [TDWG geographical codes](https://www.tdwg.org/standards/wgsrpd/) (Brummitt, 2001) expressed to that system's third level. This splits the world into 369 regions. Thus, each accepted name in POWO is associated with a list of geographic regions. POWO splits the geographic regions associated with a taxa by three conditions: -->
...
```{r do we have any endemic plants, warning=NA}
endemic = enriched_report[enriched_report$endemic == 'Endemic',]
no_endemic_plants = nrow(endemic) == 0
if(no_endemic_plants){
  warning('There are no endemic plants!')
}
```

`r if(no_endemic_plants){"There are no endemic plants."}`
`r if(no_endemic_plants){knitr::knit_exit()}`
...
In this section we explore the number of endemic plants relative to non-endemic plants in the living collection.
...
In this section we explore how the number of endemic species in the living collection has varied over time.
...
`r if(do_over_time){"Next we view endemic plants as a percentage of all plants in the living collection over time."}`
...
### Geographic coverage of endemic species
...
The maps in this section show which botanical countries are represented by endemic species in the collection, how many endemic species from each botanical country are in the collection, and the proportion of endemic species in the collection relative to the total number of endemic species in a botanical country.
...
`r if(report_kind == 'interactive'){"###### Number of endemic species held"}`
...
`r if(report_kind == 'interactive'){"###### % held relative to native endemic species"}`
...
endemic_items_per_taxa <<- endemic_items_per_taxa
...
endemic_accessions_per_taxa <<- endemic_accessions_per_taxa
...
if(nrow(report_cur) == 0){
  ...
  plots = plots_turnover(turnover, trend = trend, report_kind = report_kind, separate_figure_folder = separate_figure_folder, text = 'endemic', data_type = 'Accessions', colors = c('#f46d43', '#848484'))
  ...
  plots_proportional = proportional_plots_turnover(turnover = turnover,
                                                   ...
                                                   text = 'endemic',
                                                   data_type = 'Accessions', colors = c('#f46d43', '#848484') |> rev())
...
library(gtsummary) # Have to load library otherwise tbl_survfit fails.
...
### Taxonomic representation of endemic taxa

In this section we create sunburst and treemap charts showing all of the endemic species found within WCVP (accepted species), grouped into families, orders and higher-level lineages. The charts are shaded lighter to darker orange depending on the proportion of the endemic lineages found in the collection, where lineages in light orange are not represented. This provides a tool to visualise potential lineages to add to the collection in order to increase taxonomic diversity. 
...
data_formatted$hover = paste0(round(data_formatted$percentage*100,digits = 2),'% of endemic ', data_formatted$labels |> tolower(), ' genera found within the LC (',data_formatted$in_LC,'/',data_formatted$values,')')
