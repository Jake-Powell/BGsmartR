...
In particular this report explores the representation of different clades in the LC (Angiosperms, Monocots, etc). In this analysis we restrict the plants in the collection by removing cultivars. 
...
Any analyses over time reconstructs the existing plants in the collection on the 1st of January each year. 
...
## Has the proportion of represented plants in a lineage changed over time?

In this section we look at how the coverage of plant lineages have changed over time in the LC. 
...
to_do = c("Gymnosperms", "Eudicots",    "Monocots" ,   "Asterids",    "Rosids")
# Species percentage.
 figs = list()
...
`r if(report_kind == 'interactive'){"#####  Representation over time {.tabset}"}`

`r if(report_kind == 'interactive'){"###### Gymnosperms"}`
```{r  representation over time Gymnosperms, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
figs$Gymnosperms
```

`r if(report_kind == 'interactive'){"###### Eudicots"}`
```{r  representation over time Eudicots, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
figs$Eudicots
```

`r if(report_kind == 'interactive'){"###### Monocots"}`
```{r  representation over time Monocots, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
figs$Monocots
```

`r if(report_kind == 'interactive'){"###### Asterids"}`
```{r  representation over time Asterids, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
figs$Asterids
```

`r if(report_kind == 'interactive'){"###### Rosids"}`
```{r  representation over time Rosids, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
figs$Rosids
```
...
## What is the taxonomic composition of a LC? 

In this section we partition the taxa and accessions in the collection into: Bryophytes, Ferns, Gymnosperms, Lycophytes, Eudicots, Monocots and Other. Using the partition, we create pie charts showing the composition of the collection with respect to accessions and taxa.
...
# 
# 
# colors = viridis::viridis(nrow(taxa))
# colors_alpha = BGSmartR::add_alpha(colors, alpha = 0.5)
# zeros = rep(0,nrow(taxa))
# 
# fig1 <- plot_ly(taxa, labels = ~grouped, values = ~Freq,
#                 type = 'pie',
#                 marker = list(colors = colors_alpha),
#                 pull = rep(0,nrow(taxa)),
#                 hole =.4) %>% layout(title = 'Taxa')
# 
# 
...
# 
# 
# fig1 <- plot_ly(accessions, labels = ~grouped, values = ~Freq,
#                 type = 'pie',
#                 marker = list(colors = colors_alpha),
#                 pull = rep(0,nrow(accessions)),
#                 hole =.4) %>% layout(title = 'Accessions')
# 
# 
# fig1 =  fig1 %>% htmlwidgets::onRender(paste0("function(el, x) {
#             el.on('plotly_hover', function(d){
...
# Together
p <- plot_ly() |>
  add_pie(data = accessions, labels = ~grouped, values = ~Freq, hole = 0.6,
          title = "Accessions", name ='', sort = FALSE, marker = list(
            colors =  scales::brewer_pal(palette = palette, direction = 1)(6)),
          domain = list(row = 0, column = 0)) |>
  add_pie(data = taxa, labels = ~grouped, values = ~Freq, hole = 0.6,
...
`r if(report_kind == 'interactive'){"#####  Composition on LC {.tabset}"}`
...
DT::datatable(table_composition, rownames = F, options = list(dom = 't'))
```
...
## Has the proportion of plants in a lineage changed over time in a LC?
...
fig = plot_ly(taxa_data, x = ~year, y = ~prop, type = 'bar', colors =  scales::brewer_pal(palette = palette, direction = 1)(6),
          name = ~grouped, color = ~grouped, hovertemplate = paste('%{y:.2f}%')) %>%
  layout(yaxis = list(title = 'Proportion of the LC'),
         xaxis = list(title = 'Year'),barmode = 'stack')
fig <- fig |> layout(hovermode = 'x unified')
fig <- fig |> layout(title = paste0('Taxonomic composition of the LC over time (taxa)'))
fig <- fig |> layout(hovermode = 'x unified')
...
fig = plot_ly(accessions_data, x = ~year, y = ~prop, type = 'bar',colors =  scales::brewer_pal(palette = palette, direction = 1)(6), name = ~grouped, color = ~grouped, hovertemplate = paste('%{y:.2f}%')) %>%
  layout(yaxis = list(title = 'Proportion of the LC'),
          xaxis = list(title = 'Year'), barmode = 'stack')
fig <- fig |> layout(hovermode = 'x unified')
fig <- fig |> layout(title = paste0('Taxonomic composition of the LC over time (accessions)'))
fig <- fig |> layout(hovermode = 'x unified')

figs$accessions = fig
```

`r if(report_kind == 'interactive'){"#####  Composition over time {.tabset}"}`
...
## Has the number of taxa of major plant lineages in a collection changed over time?

```{r, echo = F,fig.fullwidth=TRUE, fig.dim = c(10, 5)}
taxa_data$year = stringr::str_extract(taxa_data$year, pattern = '[0-9]{4}')
fig = plot_ly(taxa_data, x = ~year, y = ~Freq, type = 'scatter', mode = 'lines', colors =  scales::brewer_pal(palette = palette, direction = 1)(6),
          name = ~grouped, color = ~grouped) %>%
  layout(yaxis = list(title = 'Number of taxa in the LC'))
fig <- fig |> layout(hovermode = 'x unified')
...
## Is a LC fairly representing natural taxonomic proportion of species in plant lineages or is there potential to improve the evenness component of diversity

In this section we compare the collections composition of plant lineages compared to the accepted plants within WCVP.

```{r, echo = F}
# Get the proportion of groups's number of taxa relative to the angiosperms.
wanted_wcvp = no_species_genera_wcvp[match(c('Monocots', 'Asterids', 'Rosids'), no_species_genera_wcvp$Group),]
wanted_wcvp$prop_angio_species = round(wanted_wcvp$`Number of Species` / no_species_genera_wcvp$`Number of Species`[1] *100 ,digits = 3)

# use the previously calculated number of groups found in the collection 
# (only those matched to wcvp)

# no_species_genera_collection
wanted_LC = no_species_genera_collection[match(c('Monocots', 'Asterids', 'Rosids'), no_species_genera_collection$Group),]
wanted_LC$prop_angio_species = round(wanted_LC$`Number of Species` / no_species_genera_collection$`Number of Species`[1] *100 ,digits = 3)

dfff = rbind(data.frame(where = rep('WCVP',3), in_out = rep('In Group', 3), wanted_wcvp[,c(1,4)]),
           data.frame(where = rep('WCVP',3), in_out = rep('Not in Group', 3), wanted_wcvp[,c(1,4)]),
           data.frame(where = rep('LC',3), in_out = rep('In Group', 3), wanted_LC[,c(1,4)]),
           data.frame(where = rep('LC',3), in_out = rep('Not in Group', 3), wanted_LC[,c(1,4)])
)
  
dfff$prop_angio_species[dfff$in_out == 'Not in Group']  = 100 -  dfff$prop_angio_species[dfff$in_out == 'Not in Group'] 

q <- dfff[dfff$Group == 'Monocots',] %>% 
  group_by(in_out) %>% 
  arrange(where) %>%
  plot_ly(
    x = ~where, 
    y = ~prop_angio_species, 
    color= ~in_out,
    colors = rev(color_binary),
    type = 'bar',
    legendgroup = "A", hovertemplate = paste('%{y:.2f}%'))



qq <- dfff[dfff$Group == 'Asterids',] %>% 
  group_by(in_out) %>% 
  arrange(where) %>%
  plot_ly(
    x = ~where, 
    ...
    type = 'bar',
    legendgroup = "A",
    showlegend=F, hovertemplate = paste('%{y:.2f}%')) |> 
  layout(xaxis = list(title = 'Asterids'))

qqq <- dfff[dfff$Group == 'Rosids',] %>% 
  group_by(in_out) %>% 
  arrange(where) %>%
  plot_ly(
    x = ~where, 
...
fig = subplot(q,qq,qqq,shareY = T) %>% layout(barmode = 'stack', yaxis = list(title = 'Percentage of WCVP/LC'), 
                                        margin = list( l = 50, r = 50,b = 50, t = 1, pad = 1)
                                        )
fig = fig |> layout(xaxis = list(title = 'Monocots'), xaxis2 = list(title = 'Asterids'), xaxis3 = list(title = 'Rosids'))
fig <- fig |> layout(hovermode = 'x unified')
...
## The taxonomic diversity currently in the LC

Below we produce a sunburst and treemap charts to show the taxonomic diversity of the collection. This is done at the individual (item) level such that hovering over any segment shows the number of the individuals in the collection within the group (family, genera, etc). (Note that if there is larger than one at the species level this means there are subspecies, varieties or forma held of that plant species in the collection).
...
subfamily_genus_simp <- subfamily_genus_simp[as.numeric(subfamily_genus_simp$no_genera)>1,]
...
# Remove those without a subfamily (we will deal with these later)
tribe_genus_simp = tribe_genus_simp[tribe_genus_simp$tribe != "No Tribe",]
...
## Taxonomic diversity of WCVP compared to the collection (genus level)


```{r, Create taxonomic diversity genus level, echo = FALSE, message = FALSE}
report = wcvp_og$wcvp_names
...
rbPal <- colorRampPalette(c('#f1f2f0','black'))
...
In this section we create sunburst and treemap charts showing all the genera within WCVP, where the genera are grouped into families, order and higher level lineages. The charts are shaded between white and black depending on the proportion of the lineage's genera is found in the collection. 

`r if(report_kind == 'interactive'){"#####  Genera in/ not in the LC {.tabset}"}`
