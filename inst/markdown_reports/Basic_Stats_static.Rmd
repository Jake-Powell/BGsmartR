...
general_taxonomic_div<-ggplot(data=taxa, aes(x=TaxonomicCategory, y=count)) +
  ...
  geom_text(aes(label=count), vjust=-0.3, color="black",
            position = position_dodge(0.9), size=5/14*15)+
  theme_minimal()+
 ...
 general_taxonomic_div = general_taxonomic_div + theme(axis.text=element_text(size=15),
...
TaxLev_plot<- ggplot(data = taxa_infra, aes(x=2, y=number, fill=InfragenericLevel))+
  ...
  geom_text(aes(y= lab.ypos, label = number), color = "black", size=5/14*15)+
  xlim(0.5, 2.5) + labs(fill = 'Taxonomic Level')

TaxLev_plot = TaxLev_plot +theme(legend.title = element_text(size=17),
        legend.text = element_text(size=15))
...
general_numbers = general_numbers + theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17), 
        legend.title = element_text(size=17),
        legend.text = element_text(size=15), 
        )
...
details$contain = rep('No',nrow(details))
details$contain[which(details$no_plants > 1)] = 'Yes'
...
area_polygons <- wgsrpd3
mapp = sf::st_cast(area_polygons, "MULTIPOLYGON")

# Yes/ No present map
p = ggplot(mapp) +
  geom_sf(aes(fill = details$contain)) + theme_void() + 
   theme(legend.position = "bottom")+
  scale_fill_manual(values = c("Yes"="#78c679","No"="#e6e6e6"))

p <- p + guides(fill=guide_legend(title="Present"))


# Yes/ No present map
q = ggplot(mapp) +
  geom_sf(aes(fill = details$no_unique_plants)) + theme_void() + 
   scale_fill_gradientn(colours = c("#FFFFCC","#78c679" ,"#238443"),na.value = "white", guide = "colourbar", name="Number of Unique Plants")
```
...
#### BRU level 3 areas represented by the garden's collection

```{r, echo =F, fig.dim = c(10, 4), eval = T}
...
#### Number of unique plants present from each BRU level 3 area in the garden's collection

```{r, echo =F, fig.dim = c(10, 4), eval = T}
q + theme(legend.title = element_text(size=17),
        legend.text = element_text(size=15))
```

***

<hr style="border: 3px solid green; width:90%;"></hr>
...
result = BGSmartR::exist_at_date(date, acc = enriched_report$AccYear,
...
fig = ggplot(data=data, aes(x=year, y=no_families)) +
  geom_line(colour = "#78c679",size=2)+
  theme_minimal() +
  geom_point() + labs(title="",x="Year", y = "Number of Families")
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17))
fig
```
...
fig = ggplot(data=data, aes(x=year, y=no_genus)) +
  geom_line(colour = "#78c679",size=2)+
  theme_minimal() +
  geom_point() + labs(title="",x="Year", y = "Number of Genera")
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17))
fig
...
fig = ggplot(data=data, aes(x=year, y=no_species)) +
  geom_line(colour = "#78c679",size=2)+
  theme_minimal() +
  geom_point() + labs(title="",x="Year", y = "Number of Species")
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17))
fig
...
fig = ggplot(data=data, aes(x=year, y=no_accessions)) +
  geom_line(colour = "#78c679",size=2)+
  theme_minimal() +
  geom_point() + labs(title="",x="Year", y = "Number of Accessions")
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17))
fig
...
fig = ggplot(data=data, aes(x=year, y=no_items)) +
  geom_line(colour = "#78c679",size=2)+
  theme_minimal() +
  geom_point() + labs(title="",x="Year", y = "Number of Items")
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17))
fig
...
names(no_items) = c('Indeterminant', 'Species', 'Infrageneric', 'Horticultral')
year = as.numeric(stringr::str_extract(rownames(no_items),'[0-9]{4}'))

# Re format no_items for ggplot.
dat = NULL
for(i in 1:ncol(no_items)){
  dat = rbind(dat, data.frame(year,no_items[,i], rep(names(no_items)[i], length(year) ) ))
}
names(dat) = c('year', 'no_items', 'category')

# Format for proportion pplt.
prop = round(no_items/rowSums(no_items)*100,digits=2)
dat_prop = NULL
for(i in 1:ncol(no_items)){
  dat_prop = rbind(dat_prop, data.frame(year,prop[,i], rep(names(no_items)[i], length(year) ) ))
}
names(dat_prop) = c('year', 'no_items', 'category')
...
fig = ggplot(data=dat, aes(x=year, y=no_items, colour = category)) +
  geom_line(size=1.5)+
  theme_minimal() + labs(colour = 'Taxonomic Level') + xlab('Year') + ylab('Number of Items')
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17), 
        legend.title = element_text(size=17),
        legend.text = element_text(size=15))
...
fig = ggplot(data=dat_prop, aes(x=year, y=no_items, colour = category)) +
  geom_line(size=1.5)+
  theme_minimal() + labs(colour = 'Taxonomic Level') + xlab('Year') + ylab('Proportion of Items')
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17), 
        legend.title = element_text(size=17),
        legend.text = element_text(size=15))
...
fig = ggplot(data=dat, aes(x=year, y=no_items, colour = category)) +
  geom_line(size=1.5)+
  theme_minimal() + labs(colour = 'Provenance') + xlab('Year') + ylab('Number of Items')
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17), 
        legend.title = element_text(size=17),
        legend.text = element_text(size=15))
...
fig = ggplot(data=dat_prop, aes(x=year, y=no_items, colour = category)) +
  geom_line(size=1.5)+
  theme_minimal() + labs(colour = 'Provenance') + xlab('Year') + ylab('Proportion of Items')
fig = fig +theme(axis.text=element_text(size=15),
        axis.title=element_text(size=17), 
        legend.title = element_text(size=17),
        legend.text = element_text(size=15))
