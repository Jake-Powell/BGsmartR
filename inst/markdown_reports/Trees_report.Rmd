...
```{r, check if we have any trees, warning=NA}
no_tree_plants = nrow(Tree) == 0
if(no_tree_plants){
  warning('There are no trees!')
}
```

`r if(no_tree_plants){"There are no trees."}`
`r if(no_tree_plants){knitr::knit_exit()}`

```{r get tree only information part 2}
...
AA = lapply(remove_nodes, function(node){
  ...
  if(length(to_remove)> 0 ){
    ...
    data_formatted <<- data_formatted  # This is needed don't comment
  }
...
```{r, Among the native plants in a LC, how many of them are endemic or redlisted - get important taxa, ft.keepnext = FALSE, eval = TRUE}
