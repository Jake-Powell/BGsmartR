---
title: "Overview of trees in the LC"
...
This report explores trees in `r collection`, and how the number of trees has varied over time.

A taxa in the collection is classed as a **tree** if it's genus species pair is found within [BGCI's GlobalTreeSearch](https://tools.bgci.org/global_tree_search.php), or if the matched accepted POWO taxonomic name is found within BGCI's GlobalTreeSearch.

Note that in this document we will show the geographic area that trees in the LC belong to. To determine the geographic distribution of items in the LC we enrich with information from [Plants of the World Online](https://powo.science.kew.org), by matching taxonomic names. POWO uses [TDWG geographical codes](https://www.tdwg.org/standards/wgsrpd/) (Brummitt, 2001) expressed to that system's third level. This splits the world into 369 regions. Thus, each accepted name in POWO is associated with a list of geographic regions. POWO splits the geographic regions associated with a taxa by three conditions:
...
names(Tree_count_table) = c('Kind', 'All', 'Tree', 'Proportion')
if(export_data){
...
In the LC there are `r format(no_Tree_items_existing, big.mark=',')` items corresponding to  `r format(no_Tree_accessions_existing, big.mark=',')` accessions of existing trees. The number of trees existing in the LC is summarised below. Note that here species is defined as the genus species pair and taxa is the taxonomic name and author (thus including subspecies, variety, forma).
...
In this section we explore how the number of trees in the LC has varied over time. For each year we use the accession year and item status date to determine which items in the collection are existing on the 1st of January each year.
...
`r if(report_kind == 'interactive' & do_over_time){"##### Number of trees over time  {.tabset}"}`
...
`r if(do_over_time){"Next we view trees in the LC as a percentage of all plants in the LC over time."}`
...
`r if(report_kind == 'interactive' & do_over_time){"#####  Proportion of Tree plants over time graphs {.tabset}"}`
...
if(export_data){
  ...
  exporting_data_store$map_of_Tree_species = l
  rm(l)
}

percent_rep = round(sum(plot_info$rep_Tree) / length(plot_info$rep_Tree) *100,digits = 2)
no_rep = plot_info$LEVEL3_NAM[!plot_info$rep_Tree]
```

Note that a trees may belong to multiple geographic regions. Therefore, a single taxa/item can contribute to the count of multiple regions on the map. As such if one was to add the number of items from all the regions this will be larger than the number of trees in the LC. 

Recall that we use WCVP to determine the geographic distribution of taxa and we use a combination of WCVP and BGCI's GlobalTreeSearch to determine the threatened status. Therefore, it is possible that we find a tree that is not found in WCVP, and thus we do not have a geographic distribution for these taxa. In the LC there are `r length(unique(Tree_existing$best_name[which(is.na(Tree_existing$geography_codes))]))` taxa that are classed as trees without geographic information provided by WCVP.
...
if(report_kind == 'interactive'){
  DT::datatable(Tree_count_table, rownames = FALSE)
}
```
...
`r if(report_kind == 'interactive'){"##### Maps of trees held in the LC {.tabset}"}`

`r if(report_kind == 'interactive'){"###### Has a tree from the region"}`
...
text = paste0('Region: ', plot_int_info$name, '<br />', 
              ...
              'Species in LC: ', plot_int_info$no_Tree_species, '<br />',
              'Items in LC: ', plot_int_info$no_Tree_items)
...
`r if(report_kind == 'interactive'){"###### By number of trees (species) held"}`
...
In this section, we explore the duplication of existing trees in the LC. 

Below we produce pie charts and a table of the number of trees that have 1, 2, 3, 4 or >4 items or accessions in the LC. 
...
`r if(report_kind == 'interactive'){"#####  Duplication of trees in the LC {.tabset}"}`
...
# 
# text = paste0(duplication_items$Items, ' tree Taxa has ', duplication_items$Duplication, ' items')
# 
...
# 
# if(separate_figure_folder){
#   htmlwidgets::saveWidget(fig, file = paste0(figures_dir, '/','Pie-duplication_tree_items.html'))
# }
# 
...
Tree_wanted_info = Tree_existing[c("best_name", "no_gardens")]
...
In this section, we explore the rarity of existing trees in the LC on the global scale. To do this we used BGCI's PlantSearch to extract the number of global collections plants are held within.
...
```{r, Provenance of Tree plants in the LC}
...
`r if(report_kind == 'interactive'){"#####  Provenance of trees in the LC {.tabset}"}`
...
`r if(do_over_time){"We will now look at how the provenance of trees in the LC has changed over time."}`

`r if(!do_over_time){"We do not have the information to explore how provenance of trees in the LC has changed over time."}`
...
`r if(report_kind == 'interactive'& do_over_time){"#####  Provenance of trees in the LC over time {.tabset}"}`
...
This section looks at the turnover (gain/loss) of accessions of trees in the LC.
...
if(report_kind == 'interactive'){
  ...
  fig$x$data[[2]]$name = 'Non-Tree'
  fig$x$data[[2]]$legendgroup = 'Non-Tree'
...
table_data = table_data$table_body
  ...
  table_data = table_data[-1,]
  names(table_data) = c('Tree', 'Non-Tree')
...
`r if(report_kind == 'interactive'){"##### Native, endemic and threatened trees  {.tabset}"}`
...
draw.triple.venn(
	...
	fill = viridis::viridis(3,alpha = 0.4),
	col = viridis::viridis(3),
	lwd = 1,
	cex = 1.1,
	cat.cex = 1.1,
	cat.col = viridis::viridis(3),
	cat.pos = c(-27, 27, 135),
