...
fit_species = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[which(!grepl('0|2|3|4|5|6',data_to_analyse$taxon_type)),])
...
fit_infra = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[which(!grepl('0|5|6', data_to_analyse$taxon_type) & grepl('2|3|4', data_to_analyse$taxon_type)),])
...
fit_hort = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[grep('5|6',data_to_analyse$taxon_type),])
...
fit_indet = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[grep('0',data_to_analyse$taxon_type),])
...
fit_garden = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[which(data_to_analyse$ProvenanceCode == 'Garden'),])
...
fit_wild = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[which(data_to_analyse$ProvenanceCode == 'Wild'),])
...
if("Wild-derived" %in% data_to_analyse$ProvenanceCode){
  fit_z = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[which(data_to_analyse$ProvenanceCode == 'Wild-derived'),])
...
if('Unknown' %in% data_to_analyse$ProvenanceCode){
  fit_unknown = ggsurvfit::survfit2(survival::Surv(time, Alive_dead) ~ 1,
                              data = data_to_analyse[which(data_to_analyse$ProvenanceCode == 'Unknown'),])
...
survival_table = lapply(names(median_vals), function(x){
  tb = median_vals[[x]]
  median = tb$table_body[1,6] |> as.character() #|> stringr::str_extract_all('[0-9]{1,3}') |> unlist() |> as.numeric()
  tb = surv_percent_vals[[x]]
  surv_prob = tb$table_body[1,6:8] |> as.character()
...
fig = plot_ly(data = data.frame(), type = 'scatter', mode = 'lines', x = fit_garden$time, y = fit_garden$surv, name = 'Garden', hoverinfo = "text",  hovertext = fit_garden$hover, line = list(color = colors[['Garden']]) )
...
df = report[!is.na(report$POWO_plant_name_id),]
...
column_interest = match(c('AccYear', 'LossYear'), names(df))
no_more_info = pbapply::pblapply(no_more_ids, function(x){
  ...
  return(c(plant, last_seen, total_years, mess))
}) |> data.frame() |> t() |> data.frame()
names(no_more_info) = c('Taxonomic name and author', 'Last year in collection', 'Total years in collection', 'In collection history')
...
To download all plants (found within POWO) that are no longer found in the collection together with their history in the collection use the button below

```{r link to download plants from missing endemic regions, eval = report_kind == 'interactive'}
  no_more_info |>
  download_this(
