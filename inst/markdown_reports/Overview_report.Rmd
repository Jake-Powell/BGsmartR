...
<!-- Run css_and_logo.Rmd script, initial setup and checking for issues with the enriched report  -->
```{r, setup + css + logo,  child= paste0(system.file(package = "BGSmartR"), "/report_parts/css_and_logo.Rmd")}
```

<!-- Run geography_and_LC_location.Rmd script, to create the geographic codes and find which geographic region the LC belongs to  -->
```{r, geographic details, child= paste0(system.file(package = "BGSmartR"), "/report_parts/geography_and_LC_location.Rmd")}
```

<!-- Run restructure_enriched_report.Rmd script, which creates `report` used in making plots, where additional columns are added such as threatened, native, endemic, trees, etc -->
```{r, restructure enriched report, child= paste0(system.file(package = "BGSmartR"), "/report_parts/restructure_enriched_report.Rmd")}
```
...
interest = report[which(report$ItemStatusType == 'Existing'),]
interest <<- interest
...
tree[which(interest$Enrich_is_tree |> as.logical())] = 'Tree'
interest$tree = tree
...
info = pbapply::pblapply(quant, function(q){
  ...
  if(nrow(table_counts) %in% c(1,2)){
    ...
    table_counts$colors = scales::brewer_pal(palette = colour_palette, direction = 1)(nrow(table_counts))
  }
  ...
  if(nrow(table_counts_POWO) %in% c(1,2)){
    ...
    table_counts_POWO$colors = scales::brewer_pal(palette = colour_palette, direction = 1)(nrow(table_counts_POWO))
  }
...
# Manual fix colours
colo = scales::brewer_pal(palette = colour_palette, direction = 1)(5) |> rev()
...
colours_cont = scales::brewer_pal(palette = colour_palette, direction = 1)(7)
...
<!-- Create time_series_info with the trend data. -->
```{r, restructure enriched report, child= paste0(system.file(package = "BGSmartR"), "/report_parts/prepare_trend_data.Rmd")}
```
...
colours = scales::brewer_pal(palette = colour_palette, direction = 1)(length(to_plot))
...
time_series_info_subsets <<- time_series_info_subsets
...
Endemic_categories = c("Endemic", "Not Endemic")   
...
native_categories = c("Native", "Not Native")
...
threatened_categories = c("Threatened", "Not Threatened")
...
Tree_categories = c("Tree", "Not Tree" )
...
overall_trend = time_series_info$Accessions
...
To visualise the taxonomic diversity below we produce a sunburst chart of all species held in the collection by the number of items each species contains. The species are then grouped into genera, families, and higher levels. Note that we restrict the living collection to only those that match to WCVP.
...
colours = rep('white', nrow(data_formatted))
  ...
  colo = scales::brewer_pal(palette = colour_palette, direction = 1)(length(first_level_index)) |> rev()
  ...
  data_formatted$hover = paste0( data_formatted$labels, '<br>',data_formatted$values,' items')
...
colors = scales::brewer_pal(palette = colour_palette, direction = 1)(5)
...
The Venn diagram below shows the duplication of provenance (e.g. taxa where both wild and garden provenance accessions exist in the collection) for taxa held in the collection. 

```{r, Provenance of threatened plants in the living collection,  results='hide',fig.keep='all', message=FALSE}
# Select wanted data and add needed columns
interest = report[report$ItemStatusType == 'Existing',]
interest = interest[!is.na(interest$taxon_type),]
interest$ProvenanceCode[is.na(interest$ProvenanceCode)] = 'Unknown' # Set NA provenance to Unknown.
interest <<- interest
...
prov_items = data.frame(table(interest$ProvenanceCode))
...
prov_accessions = data.frame(table(interest$ProvenanceCode[match(unique(interest$AccNoFull), interest$AccNoFull)]))
...
venn_diag = venn.diagram(wanted, filename = NULL,
                         ...
                         fill =scales::brewer_pal(palette = colour_palette, direction = 1)(3),
                         disable.logging = TRUE,
                         main.fontfamily = "arial",
                         main.fontface = "sans",
