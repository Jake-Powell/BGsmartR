---
title: "Overview of collection analysis"
...
```{r, Size of the collection, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
...
basics$colors = rep(bar_col_alpha, nrow(basics))
...
We can also view the size of the collection by the number of families, genera, species there are in the collection. This is shown below. 

```{r Current taxonomic diversity (number of Families, Genera, Species, Taxa), fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
interest = report[report$ItemStatusType == 'Existing',]
...
basics = data.frame(count = c(no_family,no_genera, no_species, no_taxa), objects = factor(x = c("Families", "Genera", "Species", "Taxa"), levels = c("Families", "Genera", "Species", "Taxa")) )
basics$colors = rep(bar_col_alpha, nrow(basics))
basics$text = paste0(format(basics$count,big.mark=','), ' ', basics$objects)
...
# Manual fix colours
comb$color[which(comb$category == 'Unique')]   = '#f46d43'  
comb$color[which(comb$category == 'Very Rare')]   = '#41ab5d'  
comb$color[which(comb$category == 'Rare')]   = '#78c679'  
comb$color[which(comb$category == 'Common')]   = '#addd8e' 
comb$color[which(comb$category == 'Very Common')]   = '#d9f0a3' 
comb$color[which(comb$category == 'Unknown')]   = '#848484'
  
comb$color[which(comb$category == 'Wild')]   = '#f46d43'
comb$color[which(comb$category == 'Wild-derived')]   = '#50c1dc'
comb$color[which(comb$category == 'Garden')]   = '#848484'
comb$color[which(comb$category == 'Unknown')]   = '#e6e6e6'
...
```{r  composition Whole collection, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
hor_stack_bar_plot
```

`r if(report_kind == 'interactive'){"###### Only those that match WCVP"}`
```{r  composition only those in WCVP, fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
hor_stack_bar_plot_POWO_only
```
...
```{r, Map represented areas by number of species (entire collection; Geography module),  fig.fullwidth=TRUE, fig.dim = c(10, 5), eval = report_kind == 'interactive'}
interest = report[report$ItemStatusType == 'Existing',]
...
### How the collection has changed over time
...
```{r, fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
...
int_plots = group_trend_plots(years, no_taxon_type, "Types of plant", report_kind = report_kind, separate_figure_folder = separate_figure_folder, data_type = 'Accessions')
...
```{r, Change over number time key-collections together (Trends module), fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
...
no_prov = no_prov[,match(c('Wild', 'Wild-derived', 'Garden', 'Unknown'), names(no_prov))] # Choose the ordering we want

int_plots = group_trend_plots(years, no_prov, "Provenance", report_kind = report_kind, separate_figure_folder = separate_figure_folder, data_type = 'Accessions')
...
#### Gain Percentage over time by provenance (Turnover module)
...
if(report_kind == 'interactive'){
  ...
  dd$colors = colour_scale[dd$group]
  dd$group
 fig = plot_ly(data = dd, x = ~year, y = ~count, color = ~group, colors = colour_scale, type = 'bar',
 ...
 fig <- fig |> layout(hovermode = 'x unified',
                         ...
                         title = "",
                         xaxis = list(title = "Year"),
                         yaxis = list (title = stringr::str_to_title('accessions')),
                         barmode = 'stack',
                         legend = list(traceorder = 'normal'))
 fig1 = fig
...
`r if(report_kind == 'interactive'){"#####  Gain by provenance {.tabset}"}`
...
To visualise the taxonomic diversity below we produce a sunburst chart of all species held in the collection by the number of individuals each species contains. The species are then grouped into genera, families and higher levels.
...
In this section we apply survival analysis to all records in the collection (including historical records) to determine the survival probability within the collection and key categories. 


```{r Overall Survival probability,  fig.fullwidth=TRUE, fig.dim = c(10, 4), eval = report_kind == 'interactive'}
interest = report
if(!is.null(old_accession_year_codes)){
  has_old_accessions_code = which(interest$AccYear %in% old_accession_year_codes)
  no_old_accessions = length(has_old_accessions_code)
...
fit_all$hover = paste0('Collection survival Probability: ', (fit_all$surv) |> round(2), '% <br> 95% conf interval: (',(fit_all$lower*100) |> round(2),  '%, ', (fit_all$upper*100) |> round(2),'%)')
...
fit_native$hover = paste0('Native survival Probability: ', (fit_native$surv) |> round(2), '% <br> 95% conf interval: (',(fit_native$lower*100) |> round(2),  '%, ', (fit_native$upper*100) |> round(2),'%)')
...
fit_endemic$hover = paste0('Endemic survival Probability: ', (fit_endemic$surv) |> round(2), '% <br> 95% conf interval: (',(fit_endemic$lower*100) |> round(2),  '%, ', (fit_endemic$upper*100) |> round(2),'%)')
...
fit_threatened$hover = paste0('Threatened survival Probability: ', (fit_threatened$surv) |> round(2), '% <br> 95% conf interval: (',(fit_threatened$lower*100) |> round(2),  '%, ', (fit_threatened$upper*100) |> round(2),'%)')
...
fit_tree$hover = paste0('Tree survival Probability: ', (fit_tree$surv) |> round(2), '% <br> 95% conf interval: (',(fit_tree$lower*100) |> round(2),  '%, ', (fit_tree$upper*100) |> round(2),'%)')
